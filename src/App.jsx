...
const KO_SCHEDULE = {
  ...
  // R32 — IDs מתאימים ל-buildR32, שעות UTC (ET+4, ישראל ET+7)
  "R32-1":  { kickoff: _utc(2026,6,28,19, 0), venue: "משחק 73 | SoFi Stadium, Los Angeles" },       // 2A vs 2B — Jun28 15:00 ET
  "R32-2":  { kickoff: _utc(2026,6,29,20,30), venue: "משחק 74 | Gillette Stadium, Boston" },        // 1E vs best 3rd — Jun29 16:30 ET
  "R32-3":  { kickoff: _utc(2026,6,30, 1, 0), venue: "משחק 75 | Estadio BBVA, Monterrey" },         // 1F vs 2C — Jun29 21:00 ET
  "R32-4":  { kickoff: _utc(2026,6,29,17, 0), venue: "משחק 76 | NRG Stadium, Houston" },            // 1C vs 2F — Jun29 13:00 ET
  "R32-5":  { kickoff: _utc(2026,6,30,21, 0), venue: "משחק 77 | MetLife Stadium, East Rutherford" },// 1I vs best 3rd — Jun30 17:00 ET
  "R32-6":  { kickoff: _utc(2026,6,30,17, 0), venue: "משחק 78 | AT&T Stadium, Dallas" },            // 2E vs 2I — Jun30 13:00 ET
  "R32-7":  { kickoff: _utc(2026,7, 1, 1, 0), venue: "משחק 79 | Estadio Azteca, Mexico City" },     // 1A vs best 3rd — Jun30 21:00 ET
  "R32-8":  { kickoff: _utc(2026,7, 1,16, 0), venue: "משחק 80 | Mercedes-Benz Stadium, Atlanta" },  // 1L vs best 3rd — Jul1 12:00 ET
  "R32-9":  { kickoff: _utc(2026,7, 2, 0, 0), venue: "משחק 81 | Levi's Stadium, Santa Clara" },     // 1D vs best 3rd — Jul1 20:00 ET
  "R32-10": { kickoff: _utc(2026,7, 1,20, 0), venue: "משחק 82 | Lumen Field, Seattle" },            // 1G vs best 3rd — Jul1 16:00 ET
  "R32-11": { kickoff: _utc(2026,7, 2,23, 0), venue: "משחק 83 | BMO Field, Toronto" },              // 2K vs 2L — Jul2 19:00 ET
  "R32-12": { kickoff: _utc(2026,7, 2,19, 0), venue: "משחק 84 | SoFi Stadium, Los Angeles" },       // 1H vs 2J — Jul2 15:00 ET
  "R32-13": { kickoff: _utc(2026,7, 3, 3, 0), venue: "משחק 85 | BC Place, Vancouver" },             // 1B vs best 3rd — Jul2 23:00 ET
  "R32-14": { kickoff: _utc(2026,7, 3,22, 0), venue: "משחק 86 | Hard Rock Stadium, Miami Gardens" },// 1J vs 2H — Jul3 18:00 ET
  "R32-15": { kickoff: _utc(2026,7, 4, 1,30), venue: "משחק 87 | Arrowhead Stadium, Kansas City" },  // 1K vs best 3rd — Jul3 21:30 ET
  "R32-16": { kickoff: _utc(2026,7, 3,18, 0), venue: "משחק 88 | AT&T Stadium, Dallas" },            // 2D vs 2G — Jul3 14:00 ET
...
const GALAXY_CARDS_RAW = [
  ...
  ["Ousmane Dembélé",  "France",    "F",  92, { PAC:94, SHO:86, PAS:82, DRI:92, DEF:38, PHY:72 }],
  ["Lamine Yamal",     "Spain",     "F",  93, { PAC:92, SHO:90, PAS:85, DRI:97, DEF:30, PHY:70 }],
  ["Michael Olise",    "France",    "F",  90, { PAC:88, SHO:84, PAS:86, DRI:92, DEF:38, PHY:70 }],
  ["Harry Kane",       "England",   "F",  90, { PAC:70, SHO:96, PAS:88, DRI:84, DEF:38, PHY:86 }],
  ["Pedri",            "Spain",     "M",  91, { PAC:78, SHO:78, PAS:90, DRI:91, DEF:65, PHY:70 }],
  ["Bruno Fernandes",  "Portugal",  "M",  90, { PAC:75, SHO:88, PAS:92, DRI:87, DEF:70, PHY:75 }],
  ["Bukayo Saka",      "England",   "F",  90, { PAC:90, SHO:86, PAS:84, DRI:90, DEF:45, PHY:72 }],
  ["Erling Haaland",   "Norway",    "F",  95, { PAC:89, SHO:96, PAS:73, DRI:87, DEF:47, PHY:88 }],
  ["Kylian Mbappé",    "France",    "F",  96, { PAC:97, SHO:92, PAS:80, DRI:95, DEF:36, PHY:78 }],
  ["Vinícius Júnior",  "Brazil",    "F",  94, { PAC:95, SHO:88, PAS:80, DRI:96, DEF:30, PHY:74 }],
  ["Vitinha",          "Portugal",  "M",  90, { PAC:78, SHO:80, PAS:91, DRI:89, DEF:78, PHY:72 }],
  ["Thibaut Courtois", "Belgium",   "GK", 93, { PAC:50, SHO:25, PAS:65, DRI:56, DEF:92, PHY:88 }],
  ["Julián Álvarez",   "Argentina", "F",  90, { PAC:87, SHO:90, PAS:82, DRI:89, DEF:50, PHY:78 }],
  ["Nuno Mendes",      "Portugal",  "D",  90, { PAC:92, SHO:65, PAS:78, DRI:85, DEF:86, PHY:80 }],
];
...
const MANUAL_RATINGS = {
  ...
  "Romelu Lukaku": 71,
  // Morocco
  "Youssef En-Nesyri": 71,
  "Sofyan Amrabat": 70,
  "Yassine Bounou": 71,
  // Other internationals
  "Khvicha Kvaratskhelia": 86,
  "Federico Valverde": 76,
  "Darwin Núñez": 71,
  "Ronald Araújo": 75,
  "Christian Pulisic": 71,
  "Weston McKennie": 70,
};
  ...
  const renderFront = () => {
    ...
    <div style={{
      ...
      background: isGalaxy
        ? cfg.bgGrad
        : isBallon
        ? "linear-gradient(165deg,#3d2800 0%,#7a5000 20%,#d4af37 40%,#fff8dc 55%,#d4af37 70%,#7a5000 85%,#3d2800 100%)"
        : `radial-gradient(ellipse at center top, ${cfg.color}33 0%, transparent 60%), ${cfg.bgGrad}`,
      backgroundSize: "150% 150%",
      backgroundPosition: "center",
      border: isBallon ? "3px solid #d4af37" : `3px solid ${cfg.color}`,
      borderRadius: 14,
      padding: 0,
      display: "flex", flexDirection: "column",
      boxShadow: animated
        ? isBallon
          ? `0 0 40px rgba(212,175,55,0.9), 0 0 80px rgba(212,175,55,0.4), 0 8px 24px rgba(0,0,0,0.5), inset 0 0 40px rgba(212,175,55,0.15)`
      ...
      overflow: "hidden",
      backfaceVisibility: "hidden",
      transform: (isGalaxy || isBallon) ? "translateZ(0)" : undefined,
      isolation: (isGalaxy || isBallon) ? "isolate" : undefined,
    }}>
      {isGalaxy && (
        <style>{`
      ...
      <div style={{
        ...
        {isBallon && BALLON_CARD_HTML[card.id] ? (
          <img
            src={BALLON_CARD_HTML[card.id]}
            alt={card.name}
            style={{
      ...
      <div style={{
        background: isBallon
          ...
          : `linear-gradient(90deg, transparent 0%, ${posInfo.color}55 50%, transparent 100%)`,
        borderTop: isBallon ? "1px solid rgba(212,175,55,0.6)" : `1px solid ${cfg.color}66`,
        padding: (isGalaxy || isBallon)
        ...
        {isBallon ? (
          <>
            <span style={{fontSize: size==="L"?14:size==="M"?10:8}}>🏅</span>
            <div style={{fontSize: size==="L"?10:size==="M"?8:6, color:"#d4af37", fontWeight:900, letterSpacing:size==="L"?2:1}}>
              {size === "S" ? "B D'OR" : "BALLON D'OR"}
            </div>
            <span style={{fontSize: size==="L"?14:size==="M"?10:8}}>🏅</span>
          </>
        ) : (
...
// ─── 🧠 QUIZ SCREEN ──────────────────────────────────────────────────────────
const QUIZ_FLAGS = [
  {flag:"🇦🇱",a:"אלבניה",options:["אלבניה", "ארמניה", "גיאורגיה", "מונטנגרו"]},
  {flag:"🇦🇩",a:"אנדורה",options:["אנדורה", "מונקו", "ליכטנשטיין", "סן מרינו"]},
  {flag:"🇦🇹",a:"אוסטריה",options:["אוסטריה", "גרמניה", "שוויץ", "הונגריה"]},
  {flag:"🇧🇾",a:"בלארוס",options:["בלארוס", "רוסיה", "אוקראינה", "ליטא"]},
  {flag:"🇧🇪",a:"בלגיה",options:["גרמניה", "הולנד", "בלגיה", "לוקסמבורג"]},
  {flag:"🇧🇦",a:"בוסניה והרצגובינה",options:["קרואטיה", "סרביה", "בוסניה והרצגובינה", "סלובניה"]},
  {flag:"🇧🇬",a:"בולגריה",options:["רומניה", "בולגריה", "מולדובה", "מקדוניה"]},
  {flag:"🇭🇷",a:"קרואטיה",options:["סרביה", "קרואטיה", "בוסניה", "סלובניה"]},
  {flag:"🇨🇾",a:"קפריסין",options:["מלטה", "קפריסין", "יוון", "לבנון"]},
  {flag:"🇨🇿",a:"צ'כיה",options:["פולין", "סלובקיה", "צ'כיה", "אוסטריה"]},
  {flag:"🇩🇰",a:"דנמרק",options:["נורווגיה", "דנמרק", "שוודיה", "פינלנד"]},
  {flag:"🇪🇪",a:"אסטוניה",options:["לטביה", "ליטא", "אסטוניה", "פינלנד"]},
  {flag:"🇫🇮",a:"פינלנד",options:["שוודיה", "נורווגיה", "פינלנד", "אסטוניה"]},
  ...
  {flag:"🇮🇸",a:"איסלנד",options:["נורווגיה", "פינלנד", "דנמרק", "איסלנד"]},
  {flag:"🇮🇪",a:"אירלנד",options:["אירלנד", "איטליה", "מקסיקו", "קוטה דיבואר"]},
  {flag:"🇮🇹",a:"איטליה",options:["צרפת", "אירלנד", "איטליה", "מקסיקו"]},
  {flag:"🇽🇰",a:"קוסובו",options:["מקדוניה", "אלבניה", "קוסובו", "בוסניה"]},
  {flag:"🇱🇻",a:"לטביה",options:["אסטוניה", "ליטא", "לטביה", "פינלנד"]},
  {flag:"🇱🇮",a:"ליכטנשטיין",options:["אנדורה", "מונקו", "ליכטנשטיין", "לוקסמבורג"]},
  {flag:"🇱🇹",a:"ליטא",options:["לטביה", "אסטוניה", "פולין", "ליטא"]},
  {flag:"🇱🇺",a:"לוקסמבורג",options:["בלגיה", "הולנד", "לוקסמבורג", "שוויץ"]},
  {flag:"🇲🇹",a:"מלטה",options:["קפריסין", "מלטה", "ישראל", "יוון"]},
  {flag:"🇲🇩",a:"מולדובה",options:["רומניה", "מולדובה", "אוקראינה", "בולגריה"]},
  {flag:"🇲🇨",a:"מונקו",options:["אנדורה", "מונקו", "ליכטנשטיין", "סן מרינו"]},
  {flag:"🇲🇪",a:"מונטנגרו",options:["סרביה", "אלבניה", "מונטנגרו", "קרואטיה"]},
  {flag:"🇳🇱",a:"הולנד",options:["לוקסמבורג", "בלגיה", "הולנד", "גרמניה"]},
  {flag:"🇲🇰",a:"מקדוניה",options:["יוון", "בולגריה", "מקדוניה", "אלבניה"]},
  ...
  {flag:"🇸🇲",a:"סן מרינו",options:["מונקו", "אנדורה", "סן מרינו", "ליכטנשטיין"]},
  {flag:"🇷🇸",a:"סרביה",options:["קרואטיה", "בוסניה", "סרביה", "מונטנגרו"]},
  {flag:"🇸🇰",a:"סלובקיה",options:["צ'כיה", "פולין", "סלובקיה", "הונגריה"]},
  {flag:"🇸🇮",a:"סלובניה",options:["קרואטיה", "סלובניה", "אוסטריה", "בוסניה"]},
  ...
  {flag:"🇬🇧",a:"בריטניה",options:["ארה\"ב", "אוסטרליה", "ניו זילנד", "בריטניה"]},
  {flag:"🇦🇷",a:"ארגנטינה",options:["אורוגוואי", "ארגנטינה", "פרגוואי", "צ'ילה"]},
  {flag:"🇧🇴",a:"בוליביה",options:["פרו", "בוליביה", "אקוודור", "פרגוואי"]},
  {flag:"🇧🇷",a:"ברזיל",options:["ארגנטינה", "ברזיל", "קולומביה", "אורוגוואי"]},
  {flag:"🇨🇦",a:"קנדה",options:["קנדה", "דנמרק", "שוויץ", "אוסטריה"]},
  {flag:"🇨🇱",a:"צ'ילה",options:["ארגנטינה", "פרו", "צ'ילה", "בוליביה"]},
  {flag:"🇨🇴",a:"קולומביה",options:["ונצואלה", "אקוודור", "קולומביה", "פרו"]},
  {flag:"🇨🇷",a:"קוסטה ריקה",options:["קוסטה ריקה", "אל סלבדור", "גואטמלה", "הונדורס"]},
  {flag:"🇨🇺",a:"קובה",options:["דומיניקנית", "קובה", "האיטי", "פוארטו ריקו"]},
  {flag:"🇩🇴",a:"הרפובליקה הדומיניקנית",options:["קובה", "האיטי", "הרפובליקה הדומיניקנית", "ג'מייקה"]},
  {flag:"🇪🇨",a:"אקוודור",options:["קולומביה", "פרו", "אקוודור", "בוליביה"]},
  {flag:"🇸🇻",a:"אל סלבדור",options:["הונדורס", "גואטמלה", "אל סלבדור", "ניקרגואה"]},
  {flag:"🇬🇹",a:"גואטמלה",options:["מקסיקו", "בליז", "גואטמלה", "הונדורס"]},
  {flag:"🇭🇹",a:"האיטי",options:["קובה", "ג'מייקה", "האיטי", "דומיניקנית"]},
  {flag:"🇭🇳",a:"הונדורס",options:["ניקרגואה", "אל סלבדור", "הונדורס", "גואטמלה"]},
  {flag:"🇯🇲",a:"ג'מייקה",options:["ברבדוס", "טרינידד", "ג'מייקה", "קובה"]},
  {flag:"🇲🇽",a:"מקסיקו",options:["מקסיקו", "איטליה", "בולגריה", "הונגריה"]},
  {flag:"🇳🇮",a:"ניקרגואה",options:["הונדורס", "אל סלבדור", "קוסטה ריקה", "ניקרגואה"]},
  {flag:"🇵🇦",a:"פנמה",options:["קוסטה ריקה", "קולומביה", "פנמה", "ונצואלה"]},
  {flag:"🇵🇾",a:"פרגוואי",options:["אורוגוואי", "בוליביה", "פרגוואי", "ארגנטינה"]},
  {flag:"🇵🇪",a:"פרו",options:["צ'ילה", "אקוודור", "פרו", "בוליביה"]},
  {flag:"🇹🇹",a:"טרינידד וטובגו",options:["ג'מייקה", "ברבדוס", "טרינידד וטובגו", "קובה"]},
  {flag:"🇺🇸",a:'ארה״ב',options:['ארה״ב', "אוסטרליה", "ניו זילנד", "בריטניה"]},
  {flag:"🇺🇾",a:"אורוגוואי",options:["ארגנטינה", "אורוגוואי", "פרגוואי", "בוליביה"]},
  {flag:"🇻🇪",a:"ונצואלה",options:["קולומביה", "ונצואלה", "ברזיל", "גיאנה"]},
  {flag:"🇩🇿",a:"אלג'יריה",options:["מרוקו", "תוניסיה", "אלג'יריה", "לוב"]},
  {flag:"🇦🇴",a:"אנגולה",options:["קונגו", "אנגולה", "זמביה", "נמיביה"]},
  {flag:"🇧🇯",a:"בנין",options:["טוגו", "גאנה", "בנין", "ניגריה"]},
  {flag:"🇧🇫",a:"בורקינה פאסו",options:["מאלי", "ניז'ר", "בורקינה פאסו", "גאנה"]},
  {flag:"🇧🇮",a:"בורונדי",options:["רואנדה", "אוגנדה", "בורונדי", "קונגו"]},
  {flag:"🇨🇲",a:"קמרון",options:["גאנה", "ניגריה", "קמרון", "סנגל"]},
  {flag:"🇨🇻",a:"כף ורדה",options:["סנגל", "גמביה", "כף ורדה", "גינאה"]},
  {flag:"🇨🇫",a:"הרפובליקה המרכז-אפריקאית",options:["צ'אד", "קמרון", "קונגו", "הרפובליקה המרכז-אפריקאית"]},
  {flag:"🇹🇩",a:"צ'אד",options:["ניג'ר", "סודאן", "צ'אד", "הרפובליקה המרכז-אפריקאית"]},
  {flag:"🇰🇲",a:"קומורוס",options:["מדגסקר", "מוריציוס", "קומורוס", "מלדיביים"]},
  {flag:"🇨🇩",a:"קונגו דמוקרטית",options:["קונגו", "אנגולה", "קונגו דמוקרטית", "זמביה"]},
  {flag:"🇨🇬",a:"רפובליקת קונגו",options:["קונגו דמוקרטית", "גבון", "רפובליקת קונגו", "קמרון"]},
  {flag:"🇩🇯",a:"ג'יבוטי",options:["סומליה", "אריתריאה", "אתיופיה", "ג'יבוטי"]},
  {flag:"🇪🇬",a:"מצרים",options:["לוב", "סודאן", "מצרים", "ירדן"]},
  {flag:"🇬🇶",a:"גינאה המשוונית",options:["קמרון", "גבון", "גינאה המשוונית", "ניגריה"]},
  {flag:"🇪🇷",a:"אריתריאה",options:["אתיופיה", "ג'יבוטי", "אריתריאה", "סומליה"]},
  {flag:"🇸🇿",a:"אסוואטיני",options:["לסוטו", "זימבבואה", "מוזמביק", "אסוואטיני"]},
  {flag:"🇪🇹",a:"אתיופיה",options:["קניה", "אריתריאה", "אתיופיה", "סומליה"]},
  {flag:"🇬🇦",a:"גבון",options:["קמרון", "קונגו", "גבון", "גינאה המשוונית"]},
  {flag:"🇬🇲",a:"גמביה",options:["סנגל", "גינאה-ביסאו", "גמביה", "מאלי"]},
  {flag:"🇬🇭",a:"גאנה",options:["ניגריה", "טוגו", "גאנה", "קוטה דיבואר"]},
  {flag:"🇬🇳",a:"גינאה",options:["סנגל", "גינאה-ביסאו", "גינאה", "סיירה לאון"]},
  {flag:"🇬🇼",a:"גינאה-ביסאו",options:["גמביה", "סנגל", "גינאה", "גינאה-ביסאו"]},
  {flag:"🇨🇮",a:"קוטה דיבואר",options:["גאנה", "ליבריה", "גינאה", "קוטה דיבואר"]},
  {flag:"🇰🇪",a:"קניה",options:["אוגנדה", "טנזניה", "קניה", "אתיופיה"]},
  {flag:"🇱🇸",a:"לסוטו",options:["אסוואטיני", "נמיביה", "לסוטו", "זימבבואה"]},
  {flag:"🇱🇷",a:"ליבריה",options:["קוטה דיבואר", "סיירה לאון", "ליבריה", "גינאה"]},
  {flag:"🇱🇾",a:"לוב",options:["תוניסיה", "אלג'יריה", "לוב", "מצרים"]},
  {flag:"🇲🇬",a:"מדגסקר",options:["מוריציוס", "קומורוס", "מדגסקר", "מוזמביק"]},
  {flag:"🇲🇼",a:"מלאווי",options:["זמביה", "מוזמביק", "טנזניה", "מלאווי"]},
  {flag:"🇲🇱",a:"מאלי",options:["בורקינה פאסו", "ניז'ר", "מאלי", "גינאה"]},
  {flag:"🇲🇷",a:"מאוריטניה",options:["מרוקו", "מאלי", "סנגל", "מאוריטניה"]},
  {flag:"🇲🇺",a:"מוריציוס",options:["מדגסקר", "קומורוס", "מוריציוס", "רי-אוניון"]},
  {flag:"🇲🇦",a:"מרוקו",options:["אלג'יריה", "תוניסיה", "מרוקו", "לוב"]},
  {flag:"🇲🇿",a:"מוזמביק",options:["זמביה", "טנזניה", "מוזמביק", "מלאווי"]},
  {flag:"🇳🇦",a:"נמיביה",options:["בוצואנה", "אנגולה", "נמיביה", "זימבבואה"]},
  {flag:"🇳🇪",a:"ניז'ר",options:["מאלי", "צ'אד", "ניגריה", "ניז'ר"]},
  {flag:"🇳🇬",a:"ניגריה",options:["קמרון", "גאנה", "ניגריה", "בנין"]},
  {flag:"🇷🇼",a:"רואנדה",options:["בורונדי", "אוגנדה", "קונגו", "רואנדה"]},
  {flag:"🇸🇹",a:"סאו טומה ופרינסיפה",options:["גינאה המשוונית", "גבון", "קומורוס", "סאו טומה ופרינסיפה"]},
  {flag:"🇸🇳",a:"סנגל",options:["גמביה", "מאוריטניה", "גינאה", "סנגל"]},
  {flag:"🇸🇱",a:"סיירה לאון",options:["ליבריה", "גינאה", "סיירה לאון", "קוטה דיבואר"]},
  {flag:"🇸🇴",a:"סומליה",options:["אתיופיה", "קניה", "ג'יבוטי", "סומליה"]},
  {flag:"🇿🇦",a:"דרום אפריקה",options:["זימבבואה", "נמיביה", "דרום אפריקה", "בוצואנה"]},
  {flag:"🇸🇸",a:"דרום סודאן",options:["סודאן", "אתיופיה", "אוגנדה", "דרום סודאן"]},
  {flag:"🇸🇩",a:"סודאן",options:["מצרים", "ליבייה", "צ'אד", "סודאן"]},
  {flag:"🇹🇿",a:"טנזניה",options:["קניה", "מוזמביק", "אוגנדה", "טנזניה"]},
  {flag:"🇹🇬",a:"טוגו",options:["בנין", "גאנה", "ניגריה", "טוגו"]},
  {flag:"🇹🇳",a:"תוניסיה",options:["לוב", "אלג'יריה", "מרוקו", "תוניסיה"]},
  {flag:"🇺🇬",a:"אוגנדה",options:["קניה", "רואנדה", "טנזניה", "אוגנדה"]},
  {flag:"🇿🇲",a:"זמביה",options:["מלאווי", "זימבבואה", "אנגולה", "זמביה"]},
  {flag:"🇿🇼",a:"זימבבואה",options:["זמביה", "בוצואנה", "מוזמביק", "זימבבואה"]},
  {flag:"🇦🇫",a:"אפגניסטן",options:["פקיסטן", "איראן", "אפגניסטן", "טג'יקיסטן"]},
  {flag:"🇦🇲",a:"ארמניה",options:["גיאורגיה", "אזרבייג'ן", "ארמניה", "טורקיה"]},
  {flag:"🇦🇿",a:"אזרבייג'ן",options:["ארמניה", "גיאורגיה", "קזחסטן", "אזרבייג'ן"]},
  {flag:"🇧🇭",a:"בחריין",options:["קטאר", "כווית", "בחריין", "עומאן"]},
  {flag:"🇧🇩",a:"בנגלדש",options:["הודו", "פקיסטן", "מיאנמר", "בנגלדש"]},
  {flag:"🇧🇹",a:"בהוטן",options:["נפאל", "בנגלדש", "בהוטן", "הודו"]},
  {flag:"🇧🇳",a:"ברוניי",options:["מלזיה", "סינגפור", "אינדונזיה", "ברוניי"]},
  {flag:"🇰🇭",a:"קמבודיה",options:["תאילנד", "לאוס", "ויאטנם", "קמבודיה"]},
  {flag:"🇨🇳",a:"סין",options:["יפן", "קוריאה", "טייוון", "סין"]},
  {flag:"🇬🇪",a:"גיאורגיה",options:["ארמניה", "אזרבייג'ן", "טורקיה", "גיאורגיה"]},
  {flag:"🇮🇳",a:"הודו",options:["פקיסטן", "בנגלדש", "סרי לנקה", "הודו"]},
  {flag:"🇮🇩",a:"אינדונזיה",options:["מלזיה", "פיליפינים", "אינדונזיה", "ברוניי"]},
  {flag:"🇮🇷",a:"איראן",options:["עיראק", "אפגניסטן", "פקיסטן", "איראן"]},
  {flag:"🇮🇶",a:"עיראק",options:["סוריה", "ירדן", "איראן", "עיראק"]},
  {flag:"🇮🇱",a:"ישראל",options:["לבנון", "ירדן", "ישראל", "קפריסין"]},
  {flag:"🇯🇵",a:"יפן",options:["קוריאה הדרומית", "סין", "טייוואן", "יפן"]},
  {flag:"🇯🇴",a:"ירדן",options:["לבנון", "סוריה", "ישראל", "ירדן"]},
  {flag:"🇰🇿",a:"קזחסטן",options:["אוזבקיסטן", "קירגיזסטן", "קזחסטן", "טורקמניסטן"]},
  {flag:"🇰🇼",a:"כווית",options:["בחריין", "קטאר", "עומאן", "כווית"]},
  {flag:"🇰🇬",a:"קירגיזסטן",options:["קזחסטן", "טג'יקיסטן", "אוזבקיסטן", "קירגיזסטן"]},
  {flag:"🇱🇦",a:"לאוס",options:["תאילנד", "קמבודיה", "ויאטנם", "לאוס"]},
  {flag:"🇱🇧",a:"לבנון",options:["סוריה", "ישראל", "ירדן", "לבנון"]},
  {flag:"🇲🇾",a:"מלזיה",options:["אינדונזיה", "סינגפור", "תאילנד", "מלזיה"]},
  {flag:"🇲🇻",a:"מלדיביים",options:["סרי לנקה", "קומורוס", "מלדיביים", "סיישל"]},
  {flag:"🇲🇳",a:"מונגוליה",options:["סין", "קזחסטן", "רוסיה", "מונגוליה"]},
  {flag:"🇲🇲",a:"מיאנמר",options:["תאילנד", "לאוס", "קמבודיה", "מיאנמר"]},
  {flag:"🇳🇵",a:"נפאל",options:["הודו", "בוטן", "בנגלדש", "נפאל"]},
  {flag:"🇰🇵",a:"קוריאה הצפונית",options:["קוריאה הדרומית", "סין", "יפן", "קוריאה הצפונית"]},
  {flag:"🇴🇲",a:"עומאן",options:["תימן", "איחוד האמירויות", "כווית", "עומאן"]},
  {flag:"🇵🇰",a:"פקיסטן",options:["הודו", "בנגלדש", "אפגניסטן", "פקיסטן"]},
  {flag:"🇵🇭",a:"פיליפינים",options:["אינדונזיה", "מלזיה", "ויאטנם", "פיליפינים"]},
  {flag:"🇶🇦",a:"קטאר",options:["בחריין", "כווית", "עומאן", "קטאר"]},
  {flag:"🇸🇦",a:"ערב הסעודית",options:["עומאן", "כווית", "תימן", "ערב הסעודית"]},
  {flag:"🇸🇬",a:"סינגפור",options:["מלזיה", "ברוניי", "אינדונזיה", "סינגפור"]},
  {flag:"🇱🇰",a:"סרי לנקה",options:["הודו", "מלדיביים", "בנגלדש", "סרי לנקה"]},
  {flag:"🇸🇾",a:"סוריה",options:["לבנון", "עיראק", "ירדן", "סוריה"]},
  {flag:"🇹🇼",a:"טייוואן",options:["יפן", "קוריאה", "סין", "טייוואן"]},
  {flag:"🇹🇯",a:"טג'יקיסטן",options:["אוזבקיסטן", "קירגיזסטן", "אפגניסטן", "טג'יקיסטן"]},
  {flag:"🇹🇭",a:"תאילנד",options:["לאוס", "קמבודיה", "מיאנמר", "תאילנד"]},
  {flag:"🇹🇱",a:"טימור-לסטה",options:["אינדונזיה", "פפואה", "פיליפינים", "טימור-לסטה"]},
  {flag:"🇹🇲",a:"טורקמניסטן",options:["קזחסטן", "אוזבקיסטן", "אפגניסטן", "טורקמניסטן"]},
  {flag:"🇦🇪",a:"איחוד האמירויות",options:["בחריין", "כווית", "קטאר", "איחוד האמירויות"]},
  {flag:"🇺🇿",a:"אוזבקיסטן",options:["קזחסטן", "טג'יקיסטן", "קירגיזסטן", "אוזבקיסטן"]},
  {flag:"🇻🇳",a:"ויאטנם",options:["לאוס", "קמבודיה", "תאילנד", "ויאטנם"]},
  {flag:"🇾🇪",a:"תימן",options:["עומאן", "ערב הסעודית", "סומליה", "תימן"]},
  {flag:"🇦🇺",a:"אוסטרליה",options:["ניו זילנד", "פפואה גינאה החדשה", "פיג'י", "אוסטרליה"]},
  {flag:"🇫🇯",a:"פיג'י",options:["ונואטו", "סמואה", "טונגה", "פיג'י"]},
  {flag:"🇳🇿",a:"ניו זילנד",options:["אוסטרליה", "פפואה", "פיג'י", "ניו זילנד"]},
  {flag:"🇵🇬",a:"פפואה גינאה החדשה",options:["אינדונזיה", "אוסטרליה", "פיג'י", "פפואה גינאה החדשה"]},
  {flag:"🇼🇸",a:"סמואה",options:["טונגה", "פיג'י", "ונואטו", "סמואה"]},
  {flag:"🇹🇴",a:"טונגה",options:["סמואה", "פיג'י", "ניו זילנד", "טונגה"]},
  {flag:"🇻🇺",a:"ונואטו",options:["פיג'י", "סמואה", "פפואה", "ונואטו"]},
  {flag:"🇹🇷",a:"טורקיה",options:["ארמניה", "גיאורגיה", "יוון", "טורקיה"]},
  {flag:"🇰🇷",a:"קוריאה הדרומית",options:["יפן", "קוריאה הצפונית", "סין", "קוריאה הדרומית"]},
  {flag:"🇧🇼",a:"בוצואנה",options:["נמיביה", "זימבבואה", "בוצואנה", "זמביה"]},
  {flag:"🇸🇨",a:"סיישל",options:["מלדיביים", "מאוריציוס", "קומורוס", "סיישל"]},
  {flag:"🇰🇾",a:"איי קיימן",options:["ג'מייקה", "ברבדוס", "איי קיימן", "האיטי"]},
  {flag:"🇮🇨",a:"איי קנריה",options:["פורטוגל", "ספרד", "מרוקו", "איי קנריה"]},
];
...
const GENERAL_Q = [
    ...
    {q:"מי זכה במונדיאל 2010?",a:"ספרד",options:["ספרד", "הולנד", "גרמניה", "ברזיל"],cat:"היסטוריה",diff:"קל"},
    {q:"מהו פנדל?",a:"11 מטר",options:["9 מטר", "11 מטר", "12 מטר", "10 מטר"],cat:"כללי",diff:"קל"},
    {q:"כמה שחקנים מותרים להחליף בגמר?",a:"5",options:["3", "4", "5", "6"],cat:"כללי",diff:"קל"},
    {q:"מה שם מגרש ברצלונה החדש?",a:"Camp Nou",options:["Camp Nou", "Spotify Camp Nou", "Nou Camp", "El Camp"],cat:"מועדונים",diff:"קל"},
    {q:"איזו נבחרת מכונה 'La Furia Roja'?",a:"ספרד",options:["ספרד", "מקסיקו", "פורטוגל", "קולומביה"],cat:"נבחרות",diff:"קל"},
    {q:"מהי הפסיקה בכדורגל לשחקן שקיבל 2 כרטיסים צהובים?",a:"אדום",options:["פסילה", "אדום", "השעיה", "אזהרה"],cat:"כללי",diff:"קל"},
    {q:"מי הוא שחקן הכינוי 'CR7'?",a:"כריסטיאנו רונאלדו",options:["כריסטיאנו רונאלדו", "קרים בנזמה", "קיליאן מבאפה", "קסאמירו"],cat:"שחקנים",diff:"קל"},
    {q:"איזו קבוצה ניצחה ליגת האלופות 2024?",a:"ריאל מדריד",options:["ריאל מדריד", "ברצלונה", "מנצ'סטר סיטי", "בייאן"],cat:"מועדונים",diff:"קל"},
    {q:"כמה אזורים יש בגביע העולם לפני נוקאאוט?",a:"שלב הבתים",options:["שלב הבתים", "שלב הנוקאאוט", "שלב הרבע", "שלב השמינית"],cat:"כללי",diff:"קל"},
    {q:"מי הוא מאמן נבחרת ברזיל לאחרונה?",a:"דורינגו",options:["טיטה", "דורינגו", "פרנאנדו דינה", "ריקארדו טייקסיירה"],cat:"מאמנים",diff:"קל"},
    {q:"כמה דקות הפסקה בין המחציות?",a:"15",options:["10", "15", "20", "12"],cat:"כללי",diff:"קל"},
    {q:"מה שם הגביע שניתן לזוכה במונדיאל?",a:"גביע ז'ול רמה",options:["גביע FIFA", "גביע ז'ול רמה", "גביע העולם", "גביע הזהב"],cat:"כללי",diff:"קל"},
    {q:"איזה מדינה ארחה את מונדיאל 2014?",a:"ברזיל",options:["ברזיל", "ארגנטינה", "קולומביה", "צ'ילה"],cat:"היסטוריה",diff:"קל"},
    {q:"מי הוא הכדורגלן הכי יקר בהיסטוריה?",a:"ניימאר",options:["מבאפה", "בל", "ניימאר", "מסי"],cat:"שיאים",diff:"קל"},
    {q:"כמה פינות יש במגרש כדורגל?",a:"4",options:["2", "4", "8", "6"],cat:"כללי",diff:"קל"},
    {q:"מי הוא הכוכב של נבחרת אנגליה?",a:"ג'וד בלינגהם",options:["מרקוס ראשפורד", "ג'וד בלינגהם", "הארי קיין", "פיל פודן"],cat:"שחקנים",diff:"קל"},
    {q:"מהו שם הכינוי של ליגת האלופות?",a:"UCL",options:["UCL", "CL", "ECL", "UEFA Cup"],cat:"כללי",diff:"קל"},
    {q:"איזה צבע הכרטיס האדום?",a:"אדום",options:["כתום", "אדום", "ורוד", "סגול"],cat:"כללי",diff:"קל"},
    {q:"מי ניצח ביורו 2016?",a:"פורטוגל",options:["צרפת", "גרמניה", "פורטוגל", "ספרד"],cat:"היסטוריה",diff:"קל"},
    {q:"כמה שחקנים בסגל המונדיאל 2026?",a:"26",options:["23", "24", "26", "30"],cat:"מונדיאל 2026",diff:"קל"},
    {q:"מהי עברת 'אופסייד'?",a:"נמצא מול הגנה ללא שחקן",options:["נמצא מול הגנה ללא שחקן", "בעיטה מחוץ לגבול", "דחיפת שחקן", "כרטיס צהוב"],cat:"כללי",diff:"קל"},
    {q:"מי הוא השוערים המפורסם של ספרד?",a:"איקר קסיאס",options:["בופון", "נוייר", "איקר קסיאס", "דה חאה"],cat:"שחקנים",diff:"קל"},
    {q:"כמה ניצחונות שווה בטבלה?",a:"3",options:["1", "2", "3", "4"],cat:"כללי",diff:"קל"},
    {q:"איזה קבוצה מכונה 'The Red Devils'?",a:"מנצ'סטר יונייטד",options:["ליברפול", "מנצ'סטר יונייטד", "אסטון וילה", "ארסנל"],cat:"מועדונים",diff:"קל"},
    {q:"מי הוא השחקן עם הכינוי 'The Pharaoh'?",a:"מוחמד סלאח",options:["אחמד מוסה", "מוחמד סלאח", "רצ'פ סוחוי", "אורגי ביוזקלין"],cat:"שחקנים",diff:"קל"},
    {q:"כמה צוותים בליגה האנגלית Premier League?",a:"20",options:["18", "20", "22", "24"],cat:"מועדונים",diff:"קל"},
    {q:"מי זכה בגביע העולם 1986?",a:"ארגנטינה",options:["ברזיל", "ארגנטינה", "גרמניה", "איטליה"],cat:"היסטוריה",diff:"קל"},
    {q:"איזה ארץ אחראית להמצאת כדורגל?",a:"אנגליה",options:["סקוטלנד", "אנגליה", "איטליה", "גרמניה"],cat:"כללי",diff:"קל"},
    {q:"מי הוא המאמן שזכה ב-8 ליגות אלופות?",a:"אף אחד - שיא הוא 4",options:["פרגוסון", "זידאן", "אנצ'לוטי", "אף אחד - שיא הוא 4"],cat:"מאמנים",diff:"קל"},
    {q:"מה שם מגרש אינטר מילאן?",a:"San Siro",options:["Allianz Arena", "San Siro", "Olimpico", "Giuseppe Meazza"],cat:"מועדונים",diff:"קל"},
    {q:"מי כבש שיא שערים בליגה האנגלית עונה אחת?",a:"אנדי קול",options:["אלן שירר", "אנדי קול", "מוחמד סלאח", "הארי קיין"],cat:"שיאים",diff:"קל"},
    {q:"כמה פעמים בייאן מינכן זכה בבונדסליגה ברצף (2013-2023)?",a:"11",options:["8", "10", "11", "9"],cat:"מועדונים",diff:"קל"},
    {q:"מי הוא שחקן הכינוי 'La Pulce'?",a:"מסי",options:["מסי", "רונאלדינהו", "נייאמר", "קאקה"],cat:"שחקנים",diff:"קל"},
    {q:"איזו ליגה שיחק בה שחקן הכינוי 'The Rocket' רוברטו קרלוס?",a:"ריאל מדריד",options:["ריאל מדריד", "ברצלונה", "ביה מינהרנו", "אינטר"],cat:"שחקנים",diff:"קל"},
    {q:"מה מספר השחקנים המינימלי למשחק?",a:"7",options:["5", "6", "7", "8"],cat:"כללי",diff:"קל"},
    {q:"מי זכה במונדיאל 1994?",a:"ברזיל",options:["איטליה", "ברזיל", "גרמניה", "ארגנטינה"],cat:"היסטוריה",diff:"קל"},
    {q:"מה שם הגביע השנתי של UEFA בין נבחרות אירופה?",a:"ליגת האומות",options:["יורו", "ליגת האומות", "גביע קונפדרציות", "Super Cup"],cat:"כללי",diff:"קל"},
    {q:"מי הוא שחקן ה-VAR שבודק?",a:"שופט ה-VAR",options:["מנהל המשחק", "שופט ה-VAR", "מנהל הקבוצה", "שופט הקו"],cat:"כללי",diff:"קל"},
    {q:"מה שם ליגת הכדורגל הצרפתית?",a:"Ligue 1",options:["Ligue 1", "Super Ligue", "Division 1", "Championnat"],cat:"כללי",diff:"קל"},
    {q:"מי הוא שחקן שנגן ב-4 גביעי עולם?",a:"לוטאר מתאוס",options:["פלה", "לוטאר מתאוס", "מסי", "קפלו"],cat:"שיאים",diff:"קל"},
    {q:"כמה שחקנים יש בקבוצת כדורגל 7 אנשים?",a:"7",options:["6", "7", "8", "9"],cat:"כללי",diff:"קל"},
    {q:"מי הוא המאמן של ריאל מדריד 2024?",a:"קרלו אנצ'לוטי",options:["זידאן", "קרלו אנצ'לוטי", "פפ גוארדיולה", "יורגן קלופ"],cat:"מאמנים",diff:"קל"},
    {q:"איזה ארץ ארחה מונדיאל 2022?",a:"קטאר",options:["UAE", "קטאר", "סעודיה", "בחריין"],cat:"היסטוריה",diff:"קל"},
    {q:"מה שם תחרות הכדורגל בין הקבוצות הטובות מכל אירופה?",a:"ליגת האלופות",options:["Europa League", "ליגת האלופות", "Conference League", "Super Cup"],cat:"כללי",diff:"קל"},
    {q:"מי זכה ב-Ballon d'Or 2023?",a:"ליאונל מסי",options:["הולאנד", "מבאפה", "ליאונל מסי", "ויניסיוס"],cat:"שחקנים",diff:"קל"},
    {q:"כמה פינות יש במגרש כדורגל?",a:"4",options:["2", "4", "6", "8"],cat:"כללי",diff:"קל"},
    {q:"איזה ארץ מסמלת את כדורגלה בצבע כחול?",a:"איטליה",options:["ברזיל", "איטליה", "פורטוגל", "ארגנטינה"],cat:"נבחרות",diff:"קל"},
    {q:"מי הוא שחקן שמשחק בפוזיציה ST?",a:"חלוץ",options:["חלוץ", "קשר", "מגן", "שוער"],cat:"כללי",diff:"קל"},
    {q:"מה שם מגרש ליברפול?",a:"Anfield",options:["Anfield", "Stamford Bridge", "Goodison Park", "Old Trafford"],cat:"מועדונים",diff:"קל"},
    {q:"מי זכה ביורו 2012?",a:"ספרד",options:["גרמניה", "איטליה", "ספרד", "פורטוגל"],cat:"היסטוריה",diff:"קל"},
    {q:"כמה ניקוד נותן תיקו?",a:"1",options:["0", "1", "2", "3"],cat:"כללי",diff:"קל"},
    {q:"מי הוא הכוכב של נבחרת פורטוגל?",a:"כריסטיאנו רונאלדו",options:["בנרדו סילבה", "ברונו פרננדז", "כריסטיאנו רונאלדו", "פקה"],cat:"שחקנים",diff:"קל"},
    {q:"מה שם מגרש מנצ'סטר יונייטד?",a:"Old Trafford",options:["Etihad", "Old Trafford", "Emirates", "Anfield"],cat:"מועדונים",diff:"קל"},
    {q:"מי הוא שחקן שמסמל את מנצ'סטר סיטי?",a:"ארלינג הולאנד",options:["קווין דה ברויינה", "ארלינג הולאנד", "פיל פודן", "ברנרדו סילבה"],cat:"שחקנים",diff:"קל"},
    {q:"מה שם הפרס לאחרות שניצחת ב-UEFA Champions League?",a:"גביע ביג אייר",options:["גביע ביג אייר", "גביע UEFA", "טרופי ליגת האלופות", "הגביע הגדול"],cat:"כללי",diff:"קל"},
    {q:"מי ניצח ב-Copa del Rey 2023?",a:"ריאל מדריד",options:["ברצלונה", "ריאל מדריד", "אתלטיקו", "סוויה"],cat:"מועדונים",diff:"קל"},
    {q:"מה שם הליגה האיטלקית?",a:"Serie A",options:["Serie A", "Seria", "Lega Calcio", "Italian League"],cat:"כללי",diff:"קל"},
    {q:"מי הוא שחקן שנגן לפיינבהצ'ה?",a:"ג'יי ג'יי אוקוצ'ה",options:["ג'יי ג'יי אוקוצ'ה", "דיאמה", "קאנו", "אטו"],cat:"שחקנים",diff:"קל"},
    {q:"כמה כרטיסים צהובים גורמים לפסילה?",a:"2 באותו משחק",options:["1", "2 באותו משחק", "3 בטורניר", "2 בטורניר"],cat:"כללי",diff:"קל"},
    {q:"מי הוא שוער הנבחרת הגרמנית המפורסם?",a:"מנואל נוייר",options:["כהן", "בכמן", "מנואל נוייר", "לנו"],cat:"שחקנים",diff:"קל"},
    {q:"איזה ארץ ניצחה ב-Copa America 2021?",a:"ארגנטינה",options:["ברזיל", "ארגנטינה", "קולומביה", "אורוגוואי"],cat:"היסטוריה",diff:"קל"},
    {q:"מה הגיל של מסי כשזכה ראשון ב-Ballon d'Or?",a:"22",options:["20", "22", "24", "19"],cat:"שחקנים",diff:"קל"},
    {q:"מי הוא שחקן המיקום CM?",a:"קשר מרכזי",options:["חלוץ", "קשר מרכזי", "מגן ימין", "שוער"],cat:"כללי",diff:"קל"},
    {q:"מה שם מגרש אתלטיקו מדריד?",a:"Metropolitano",options:["Santiago Bernabéu", "Metropolitano", "Wanda", "Calderon"],cat:"מועדונים",diff:"קל"},
    {q:"מי זכה בגביע אפריקה 2024?",a:"קייפ ורד",options:["נייגריה", "מצרים", "קייפ ורד", "מרוקו"],cat:"היסטוריה",diff:"קל"},
    {q:"מה שם שחקן שנגן לאיבר-מנצ'סטר?",a:"רויין ואן פרסי",options:["גיגס", "רויין ואן פרסי", "הייסי", "פורלאן"],cat:"שחקנים",diff:"קל"},
    {q:"כמה מדינות יש ב-UEFA?",a:"55",options:["48", "55", "60", "50"],cat:"כללי",diff:"קל"},
    {q:"מי זכה ב-Serie A 2024?",a:"אינטר מילאן",options:["יובנטוס", "אינטר מילאן", "מילאן", "נאפולי"],cat:"מועדונים",diff:"קל"},
    {q:"מה הכינוי של ארסנל?",a:"The Gunners",options:["The Gunners", "The Reds", "The Blues", "The Citizens"],cat:"מועדונים",diff:"קל"},
    {q:"איזה שחקן ידוע בשם 'O Fenomeno'?",a:"רונאלדו נצ'ריו",options:["רונאלדינהו", "רובינייו", "רונאלדו נצ'ריו", "אדמילסון"],cat:"שחקנים",diff:"קל"},
    {q:"מה שם הליגה הפורטוגלית?",a:"Primeira Liga",options:["Super Liga", "Primeira Liga", "Liga NOS", "Portugal League"],cat:"כללי",diff:"קל"},
    {q:"מי הוא שחקן שמשחק בפוזיציה LW?",a:"כנף שמאל",options:["כנף ימין", "כנף שמאל", "מגן שמאל", "קשר שמאל"],cat:"כללי",diff:"קל"},
    {q:"כמה ליגות אלופות זכה ריאל מדריד?",a:"15",options:["12", "13", "14", "15"],cat:"מועדונים",diff:"קל"},
    {q:"מה שם הסיבוב הראשון במונדיאל 2026?",a:"שלב 32 קבוצות",options:["שלב 48 קבוצות", "שלב 32 קבוצות", "שלב 64 קבוצות", "שלב הבתים"],cat:"מונדיאל 2026",diff:"קל"},
    {q:"מי הוא שחקן שנגן לפי-אס-ג'י?",a:"קיליאן מבאפה",options:["ברנרדו", "קיליאן מבאפה", "פגבא", "לוזי"],cat:"שחקנים",diff:"קל"},
    {q:"כמה דקות מחצית שנייה?",a:"45",options:["40", "45", "50", "60"],cat:"כללי",diff:"קל"},
    {q:"מה שם מגרש יובנטוס?",a:"Allianz Stadium",options:["Juventus Arena", "Allianz Stadium", "Fiat Stadium", "Torino Arena"],cat:"מועדונים",diff:"קל"},
    {q:"מי ניצח ב-Europa League 2024?",a:"אטלנטה",options:["אטלנטה", "בייאר", "מרסיי", "ריאל סוסיאדד"],cat:"מועדונים",diff:"קל"},
    {q:"מה הכינוי של צ'לסי?",a:"The Blues",options:["The Blues", "The Pensioners", "The Lions", "The Kings"],cat:"מועדונים",diff:"קל"},
    {q:"מי הוא שחקן שנגן לגנגים?",a:"לודוביק גיולי",options:["לודוביק גיולי", "בלוטי", "ג'קה", "פיניי"],cat:"שחקנים",diff:"קל"},
    {q:"כמה נבחרות בקבוצת מונדיאל 2026?",a:"4",options:["3", "4", "5", "6"],cat:"מונדיאל 2026",diff:"קל"},
    {q:"מה שם הדרבי בין ריאל מדריד לברצלונה?",a:"El Clasico",options:["El Clasico", "Derby", "El Derbi", "Grand Match"],cat:"מועדונים",diff:"קל"},
    {q:"מי הוא השחקן שנגן לנבחרות בלגיה הכי שנים?",a:"ג'אן וורטונגן",options:["לוקאקו", "מרטינז", "ג'אן וורטונגן", "קומפני"],cat:"שחקנים",diff:"קל"},
    {q:"מה שם הכינוי של המגן הישר במגן 4?",a:"סטופר",options:["לטרל", "סטופר", "ליברו", "פיבוט"],cat:"כללי",diff:"קל"},
    {q:"מי הוא שחקן שנגן ב-Inter Miami?",a:"ליאונל מסי",options:["דיוויד בקהם", "ליאונל מסי", "רוני", "גטוסו"],cat:"שחקנים",diff:"קל"},
    {q:"מה שם ליגת הכדורגל בנדרלנד?",a:"Eredivisie",options:["Dutch League", "Eredivisie", "Holland Cup", "Super Eredivisie"],cat:"כללי",diff:"קל"},
    {q:"מי זכה ב-Ballon d'Or 2022?",a:"קרים בנזמה",options:["מסי", "מבאפה", "קרים בנזמה", "לבנדובסקי"],cat:"שחקנים",diff:"קל"},
    {q:"כמה גולים קיבל ה-GK הטוב ב-2024?",a:"לא ידוע סטטיסטיקה מדויקת",options:["0", "5", "10", "לא ידוע סטטיסטיקה מדויקת"],cat:"כללי",diff:"קל"},
    {q:"מה שם הדרבי בין מנצ'סטר סיטי לידס?",a:"The Rose Derby",options:["The Manchester Derby", "The Rose Derby", "Northern Derby", "Yorkshire Derby"],cat:"מועדונים",diff:"קל"},
    {q:"מי ניצח במונדיאל 1966?",a:"אנגליה",options:["ברזיל", "גרמניה", "אנגליה", "ארגנטינה"],cat:"היסטוריה",diff:"קל"},
    {q:"מה הכינוי של נבחרת ברזיל?",a:"Seleção",options:["Canarinho", "Seleção", "Verde-Amarela", "Canarinha"],cat:"נבחרות",diff:"קל"},
    {q:"מי הוא שחקן שמשחק בפוזיציה CB?",a:"מגן מרכזי",options:["מגן מרכזי", "חלוץ מרכזי", "קשר מרכזי", "שוער"],cat:"כללי",diff:"קל"},
    {q:"כמה ניקוד נותנת זכייה בבית במונדיאל?",a:"3",options:["1", "2", "3", "4"],cat:"כללי",diff:"קל"},
    {q:"מה שם המגרש של מנצ'סטר סיטי?",a:"Etihad Stadium",options:["City of Manchester", "Etihad Stadium", "Maine Road", "Eastlands"],cat:"מועדונים",diff:"קל"},
    {q:"מי הוא שחקן שנגן ב-Al Nassr?",a:"כריסטיאנו רונאלדו",options:["כריסטיאנו רונאלדו", "בנזמה", "קאנטה", "מנדי"],cat:"שחקנים",diff:"קל"},
    {q:"מה שם הדרבי בין אינטר לאיי-סי מילאן?",a:"Derby della Madonnina",options:["Derby della Madonnina", "Milan Derby", "Calcio Derby", "North Derby"],cat:"מועדונים",diff:"קל"},
    {q:"מי ניצח ביורו 2008?",a:"ספרד",options:["גרמניה", "ספרד", "הולנד", "רוסיה"],cat:"היסטוריה",diff:"קל"},
    {q:"מה שם שחקן שהיה קפטן של ברצלונה לאורך שנים רבות?",a:"שאבי",options:["פויול", "שאבי", "אינייסטה", "ואלדס"],cat:"שחקנים",diff:"קל"},
    {q:"כמה קבוצות ישתתפו במונדיאל 2026?",a:"48",options:["32", "40", "48", "64"],cat:"מונדיאל 2026",diff:"קל"},
    {q:"מי הוא שוער שנגן לנבחרת ספרד שנים רבות?",a:"איקר קסיאס",options:["דה חאה", "קאפדביה", "בקארד", "איקר קסיאס"],cat:"שחקנים",diff:"קל"},
    {q:"מה שם הליגה הגרמנית השנייה?",a:"2. Bundesliga",options:["3. Bundesliga", "Bundesliga 2", "2. Bundesliga", "Deutsche Liga"],cat:"כללי",diff:"קל"},
    {q:"מי הוא שחקן שנגן לשף'ילד?",a:"ג'יימי וארדי",options:["ג'יימי וארדי", "מארצ'ה", "מק'ברירד", "אדיבאיו"],cat:"שחקנים",diff:"קל"},
    {q:"מה שם מגרש הנבחרת האנגלית?",a:"Wembley",options:["Wembley", "City of London", "Arsenal", "Stamford"],cat:"נבחרות",diff:"קל"},
    {q:"מי כבש את השיא שערים בבונדסליגה עונה אחת?",a:"גרד מולר",options:["לבנדובסקי", "גרד מולר", "הולאנד", "בייזן"],cat:"שיאים",diff:"בינוני"},
    {q:"כמה שחקנים נסחרו בין ריאל לברצלונה?",a:"שאלה אמביוולנטית - יש מספר",options:["0", "3", "5", "שאלה אמביוולנטית - יש מספר"],cat:"מועדונים",diff:"בינוני"},
    {q:"מי ניצח ב-FA Cup 2024?",a:"מנצ'סטר יונייטד",options:["מנצ'סטר סיטי", "מנצ'סטר יונייטד", "צ'לסי", "ליברפול"],cat:"מועדונים",diff:"בינוני"},
    {q:"כמה שיאי שערים שבר הולאנד ב-Champions League עונה 2022/23?",a:"12",options:["10", "11", "12", "14"],cat:"שיאים",diff:"בינוני"},
    {q:"מי זכה ב-Carabao Cup 2024?",a:"צ'לסי",options:["ליברפול", "מנצ'סטר סיטי", "צ'לסי", "ארסנל"],cat:"מועדונים",diff:"בינוני"},
    {q:"מהו שם שחקן שזכה בכל הגביעים האפשריים עם קבוצה אחת?",a:"פריסיה",options:["שאבי", "פריסיה", "מסי", "פרדיה"],cat:"שחקנים",diff:"בינוני"},
    {q:"מי הוא השחקן עם הכי הרבה הופעות ב-Premier League?",a:"גארת' בארי",options:["ריאן גיגס", "גארת' בארי", "פרנק למפרד", "סטיבן ג'רארד"],cat:"שיאים",diff:"בינוני"},
    {q:"כמה שחקנים נבחרים לרשימה ה-PFA Team of the Year?",a:"11",options:["10", "11", "15", "20"],cat:"כללי",diff:"בינוני"},
    {q:"מי הוא שחקן שנגן ל-3 מועדונים שונים ב-Serie A?",a:"שאלה כללית מדי",options:["מסי", "שאלה כללית מדי", "בנזמה", "קאבני"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה שחקנים מצוות אחד יכולים לשחק בגמר כוס העולם?",a:"11",options:["10", "11", "12", "13"],cat:"כללי",diff:"בינוני"},
    {q:"מי הוא שחקן שזכה ב-Champions League עם 2 קבוצות שונות?",a:"קלאודיו ראניירי",options:["קלאודיו ראניירי", "בקהם", "לוי קולאה", "בנזמה"],cat:"שחקנים",diff:"בינוני"},
    {q:"מה שם האצטדיון של PSG?",a:"Parc des Princes",options:["Stade de France", "Parc des Princes", "Stade Vélodrome", "Stade Louis II"],cat:"מועדונים",diff:"בינוני"},
    {q:"מי ניצח ב-Copa Libertadores 2024?",a:"בוטאפוגו",options:["פלמנגו", "בוטאפוגו", "ריברפלייט", "בוקה"],cat:"מועדונים",diff:"בינוני"},
    {q:"מהו שיא השערים שכבש שחקן בגמר בודד?",a:"3",options:["2", "3", "4", "5"],cat:"שיאים",diff:"בינוני"},
    {q:"מי הוא מנהל הספורט הראשי ב-FIFA?",a:"ג'אני אינפנטינו",options:["ג'ואו אבלאנז'", "ז'וזף בלאטר", "ג'אני אינפנטינו", "מישל פלטיני"],cat:"כללי",diff:"בינוני"},
    {q:"כמה דקות לפחות צריך לשחק שחקן כדי להחשב כמשתתף?",a:"1",options:["0", "1", "5", "45"],cat:"כללי",diff:"בינוני"},
    {q:"מי הוא שחקן שכבש שיא של 5 שערים במשחק UEFA Champions League?",a:"לוקאס פודולסקי",options:["לוקאס פודולסקי", "מסי", "הולאנד", "רונאלדו"],cat:"שיאים",diff:"בינוני"},
    {q:"כמה פעמים נשא מסי את מדי ברצלונה?",a:"778",options:["600", "700", "778", "800"],cat:"שחקנים",diff:"בינוני"},
    {q:"מי ניצח ב-African Cup of Nations 2024?",a:"קייפ ורד",options:["נייגריה", "מרוקו", "קייפ ורד", "מצרים"],cat:"היסטוריה",diff:"בינוני"},
    {q:"מה שם שחקן אנגלי שנחשב לטוב בתקופתו בשנות ה-90?",a:"אלן שירר",options:["אלן שירר", "אנדי קול", "לס פרדינד", "פיטר בנד"],cat:"שחקנים",diff:"בינוני"},
    {q:"מי הוא מאמן שזכה ב-Champions League עם 3 קבוצות שונות?",a:"קרלו אנצ'לוטי",options:["פפ גוארדיולה", "זינדין זידאן", "קרלו אנצ'לוטי", "יורגן קלופ"],cat:"מאמנים",diff:"בינוני"},
    {q:"כמה שחקנים כבשו 50+ שערים ב-Premier League?",a:"שאלה קשה לאמת",options:["5", "10", "15", "שאלה קשה לאמת"],cat:"שיאים",diff:"בינוני"},
    {q:"מהו שם שחקן שנגן לאייאקס ולריאל מדריד?",a:"קלאנס סיידורף",options:["קלאנס סיידורף", "ז'ונינהו", "ריבאלדו", "אוברמארס"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה שנים ספרד שלטה ביורו?",a:"3 ניצחונות 2008-2012",options:["2008-2012", "2004-2016", "2008-2016", "3 ניצחונות 2008-2012"],cat:"נבחרות",diff:"בינוני"},
    {q:"מי ניצח ב-Ligue 1 2024?",a:"PSG",options:["מרסיי", "מונאקו", "ליל", "PSG"],cat:"מועדונים",diff:"בינוני"},
    {q:"מה הכינוי של קבוצת אייאקס?",a:"The Godenzonen",options:["The Dutch", "Ajax Boys", "The Godenzonen", "Amsterdam Kings"],cat:"מועדונים",diff:"בינוני"},
    {q:"כמה שערים כבש מסי לברצלונה בכל התחרויות?",a:"672",options:["600", "650", "672", "700"],cat:"שחקנים",diff:"בינוני"},
    {q:"מי הוא השחקן שפתח את עידן ה-Tiki-Taka?",a:"שאבי",options:["שאבי", "אינייסטה", "גואי", "ביסקטס"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה קבוצות השתתפו ב-Copa Libertadores 2024?",a:"47",options:["32", "40", "47", "64"],cat:"מועדונים",diff:"בינוני"},
    {q:"מי הוא שחקן בלגי שנחשב לאחד הטובים בדורו?",a:"קווין דה ברויינה",options:["לוקאקו", "קווין דה ברויינה", "הזארד", "קומפני"],cat:"שחקנים",diff:"בינוני"},
    {q:"מה שם שחקן שנחשב לאחד הגדולים של הולנד?",a:"יוהאן קרויף",options:["מרקו ואן באסטן", "רוד גוליט", "יוהאן קרויף", "אריאן רובן"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה כוכבות יש על חולצת ברזיל?",a:"5",options:["4", "5", "6", "3"],cat:"נבחרות",diff:"בינוני"},
    {q:"מי ניצח ב-Bundes Liga 2024?",a:"בייאן מינכן",options:["דורטמונד", "לברקוזן", "בייאן מינכן", "לייפציג"],cat:"מועדונים",diff:"בינוני"},
    {q:"מה שם שחקן שנגן ל-Arsenal 2003-2019?",a:"סאנטי קאזורלה",options:["פדזקאשקי", "טיאגו", "סאנטי קאזורלה", "אואן הריס"],cat:"שחקנים",diff:"בינוני"},
    {q:"מי הוא שחקן שכבש שיא שערים ב-Bundesliga?",a:"גרד מולר",options:["רובן לוואנדובסקי", "גרד מולר", "ניקלס פולקרוג", "פישר"],cat:"שיאים",diff:"בינוני"},
    {q:"כמה ניצחו ריאל מדריד את ברצלונה ב-El Clasico בהיסטוריה?",a:"ריאל ניצחה יותר",options:["ריאל ניצחה יותר", "ברצלונה ניצחה יותר", "שיוויון", "לא ידוע"],cat:"מועדונים",diff:"בינוני"},
    {q:"מי ניצח ב-Eredivisie 2024?",a:"PSV",options:["אייאקס", "פנאתינאיקוס", "PSV", "פיינורד"],cat:"מועדונים",diff:"בינוני"},
    {q:"מה שם שחקן שנגן לנבחרת ארגנטינה ב-3 מונדיאלים?",a:"כמה שחקנים - מסי הוא דוגמה",options:["מסי בלבד", "כמה שחקנים - מסי הוא דוגמה", "אורטגה", "סאבלה"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה שחקנים קיבלו אדום בגמר מונדיאל?",a:"3",options:["1", "2", "3", "4"],cat:"שיאים",diff:"בינוני"},
    {q:"מי הוא שחקן שנגן ל-Barcelona ו-Juventus?",a:"לואיס סוארז",options:["הנרי", "לואיס סוארז", "בוגבה", "קאנייסה"],cat:"שחקנים",diff:"בינוני"},
    {q:"מה שם שחקן שנגן לנבחרת קרואטיה ב-1998?",a:"דוור שוקר",options:["מודריץ", "שוקר", "דוור שוקר", "פרישיץ"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה שחקנים שיחקו בכל 4 גמרי ה-Champions League 2005-2011?",a:"מעטים",options:["0", "מעטים", "10", "15"],cat:"שיאים",diff:"בינוני"},
    {q:"מי ניצח ב-Primeira Liga 2024?",a:"ספורטינג ליסבון",options:["בנפיקה", "פורטו", "ספורטינג ליסבון", "ברגה"],cat:"מועדונים",diff:"בינוני"},
    {q:"כמה גמרים ב-Champions League הסתיימו בלי שערים?",a:"3",options:["1", "2", "3", "5"],cat:"שיאים",diff:"בינוני"},
    {q:"מי הוא שחקן שנחשב לגדול ביותר של אפריקה?",a:"דידייה דרוגבה",options:["רשיד יקיני", "דידייה דרוגבה", "ג'ורג' ווה", "שמואל אטו"],cat:"שחקנים",diff:"בינוני"},
    {q:"מה שם שחקן שנגן ל-Manchester United 20+ שנה?",a:"ריאן גיגס",options:["גארי נוויל", "ריאן גיגס", "פול שולס", "פיטר שמייכל"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה שחקנים כבשו 40+ שערים ב-Premier League עונה אחת?",a:"0",options:["0", "1", "2", "3"],cat:"שיאים",diff:"בינוני"},
    {q:"מי הוא שחקן שנחשב לשיא מסי שיגיע?",a:"לאמין יאמאל",options:["מבאפה", "הולאנד", "לאמין יאמאל", "גאבי"],cat:"שחקנים",diff:"בינוני"},
    {q:"מה שם שחקן שנגן ל-Ajax ול-Bayern?",a:"ניגל דה יונג",options:["ניגל דה יונג", "רובן", "ואן דייק", "בלינד"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה שחקנים ישראלים שיחקו ב-Premier League?",a:"קומץ",options:["0", "קומץ", "10", "20"],cat:"כללי",diff:"בינוני"},
    {q:"מי הוא שחקן שנגן ל-Barcelona ו-Real Madrid?",a:"רונאלדו נצ'ריו",options:["ניימאר", "לואיס פיגו", "רונאלדו נצ'ריו", "רוב"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה מדינות ב-South America Football Federation?",a:"10",options:["8", "10", "12", "14"],cat:"כללי",diff:"בינוני"},
    {q:"מי הוא שחקן שנחשב לגדול ב-Netherlands?",a:"יוהאן קרויף",options:["ואן ניסטרוי", "יוהאן קרויף", "ולן בנד", "סנייהדר"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה פעמים Liverpool זכה ב-UEFA Champions League?",a:"6",options:["5", "6", "7", "8"],cat:"מועדונים",diff:"בינוני"},
    {q:"מי ניצח ב-La Liga 2024?",a:"ריאל מדריד",options:["ברצלונה", "אתלטיקו", "ריאל מדריד", "גירונה"],cat:"מועדונים",diff:"בינוני"},
    {q:"מה שם שחקן שנגן ל-Arsenal ו-Barcelona?",a:"סילהי",options:["הנרי", "סילהי", "פאברגס", "קנטה"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה ניצחה גרמניה את ברזיל ב-7-1 2014?",a:"פעם אחת בלבד",options:["פעמיים", "פעם אחת בלבד", "שלוש פעמים", "אף פעם"],cat:"היסטוריה",diff:"בינוני"},
    {q:"מי הוא שחקן שנגן ל-PSG ו-Real Madrid?",a:"קארים בנזמה",options:["קארים בנזמה", "ג'ניסון", "גאלו", "דגבה"],cat:"שחקנים",diff:"בינוני"},
    {q:"מה שם שחקן שנחשב לאחד הגדולים של איטליה?",a:"פרנצ'סקו טוטי",options:["אלסנדרו דל פיארו", "פרנצ'סקו טוטי", "ג'אנלואיג'י בופון", "לוקה מודריץ"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה ניצחות של מסי מול רונאלדו?",a:"תוצאה שנויה במחלוקת",options:["מסי יותר", "רונאלדו יותר", "שיוויון", "תוצאה שנויה במחלוקת"],cat:"שחקנים",diff:"בינוני"},
    {q:"מי ניצח ב-Serie A 2023?",a:"נאפולי",options:["יובנטוס", "אינטר", "נאפולי", "מילאן"],cat:"מועדונים",diff:"בינוני"},
    {q:"מה שם שחקן שנגן ל-Chelsea ו-Bayern Munich?",a:"דידייה דרוגבה",options:["דמבה בה", "דידייה דרוגבה", "אוסקר", "מאטה"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה שחקנים קיבלו ה-Ballon d'Or פעמיים ברצף?",a:"כמה - ביניהם מסי ורונאלדו",options:["0", "1", "כמה - ביניהם מסי ורונאלדו", "5"],cat:"שיאים",diff:"בינוני"},
    {q:"מי הוא שחקן שנגן ל-Juventus ו-Arsenal?",a:"פטריק ויירה",options:["פטריק ויירה", "כנפרידו", "ת'יירי אנרי", "פריים"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה שחקנים ניצחו ה-Champions League עם 2 קבוצות שונות?",a:"מספר שחקנים",options:["0", "מספר שחקנים", "1", "3"],cat:"שיאים",diff:"בינוני"},
    {q:"מה שם שחקן שנגן ל-Barcelona ו-PSG?",a:"ניימאר",options:["ניימאר", "ח'אבי", "אינייסטה", "מסי"],cat:"שחקנים",diff:"בינוני"},
    {q:"מי ניצח ב-Bundesliga 2023?",a:"בייאן מינכן",options:["דורטמונד", "לברקוזן", "RB לייפציג", "בייאן מינכן"],cat:"מועדונים",diff:"בינוני"},
    {q:"כמה שנים ריאל מדריד לא זכה ב-Champions League?",a:"12 שנים 2002-2014",options:["5", "8", "12 שנים 2002-2014", "20"],cat:"מועדונים",diff:"בינוני"},
    {q:"מי הוא שחקן שנחשב לטוב ביותר בדורו 2010-2015?",a:"ליאונל מסי",options:["כריסטיאנו רונאלדו", "ליאונל מסי", "ניימאר", "מודריץ"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה ניצחות בבית של מנצ'סטר יונייטד ב-Premier League?",a:"לא מדויק ללא מחקר",options:["100", "200", "300", "לא מדויק ללא מחקר"],cat:"מועדונים",diff:"בינוני"},
    {q:"מי ניצח ב-Eredivisie 2023?",a:"אייאקס",options:["PSV", "פיינורד", "אייאקס", "אלקמאר"],cat:"מועדונים",diff:"בינוני"},
    {q:"מה שם שחקן שנגן ל-Juventus ל-20 שנה?",a:"אלסנדרו דל פיארו",options:["גיאנפרנקו זולה", "אלסנדרו דל פיארו", "פאביו קנאוורו", "אנדראה פירלו"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה שחקנים ניצחו ה-World Cup כמשחקנים ומאמנים?",a:"מעטים",options:["0", "מעטים", "10", "20"],cat:"שיאים",diff:"בינוני"},
    {q:"מי הוא שחקן שנגן ל-Atletico Madrid ו-Liverpool?",a:"פרנן טורס",options:["קונה", "פרנן טורס", "ג'ורדי אלבה", "סאול"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה שנים ה-Invincibles של ארסנל קיימו ב-Premier League?",a:"2003/04 - עונה אחת",options:["2003/04 - עונה אחת", "2002-2004", "2003-2005", "3 עונות"],cat:"מועדונים",diff:"בינוני"},
    {q:"מי ניצח ב-Primera Liga Portuguesa 2023?",a:"בנפיקה",options:["פורטו", "ספורטינג", "בנפיקה", "ברגה"],cat:"מועדונים",diff:"בינוני"},
    {q:"מה שם שחקן שנגן ל-Real Madrid ו-Bayern Munich?",a:"קפלו",options:["גוטי", "בנדנר", "קפלו", "מאקה"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה שחקנים ניצחו ה-Champions League 3 פעמים ברצף?",a:"קבוצת ריאל מדריד 2016-2018",options:["0", "1", "קבוצת ריאל מדריד 2016-2018", "5"],cat:"שיאים",diff:"בינוני"},
    {q:"מי הוא שחקן שנגן ל-Bayern ו-Barcelona?",a:"פייר עמריק אובמיאנג",options:["ריבאלדו", "פייר עמריק אובמיאנג", "קוטינהו", "לוקאס פורה"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה שחקנים ניצחו ה-World Cup ב-2022?",a:"ארגנטינה 26 שחקנים",options:["23", "26", "ארגנטינה 26 שחקנים", "30"],cat:"היסטוריה",diff:"בינוני"},
    {q:"מי ניצח ב-Ligue 1 2023?",a:"PSG",options:["מרסיי", "ליל", "PSG", "מונאקו"],cat:"מועדונים",diff:"בינוני"},
    {q:"כמה ניצחות של Liverpool ב-Premier League?",a:"0 - ניצחו פעמים אחדות לפני",options:["8", "0 - ניצחו פעמים אחדות לפני", "1", "3"],cat:"מועדונים",diff:"בינוני"},
    {q:"מה שם שחקן שנגן ל-Juventus ו-Real Madrid?",a:"פאביו קנאוורו",options:["קארלס פויול", "פאביו קנאוורו", "סרחיו רמוס", "ג'רארד פיקה"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה שחקנים ניצחו ה-Ballon d'Or עם PSG?",a:"0",options:["0", "1", "2", "3"],cat:"שיאים",diff:"בינוני"},
    {q:"מי ניצח ב-La Liga 2023?",a:"ברצלונה",options:["ריאל מדריד", "אתלטיקו", "ברצלונה", "ביאריאל"],cat:"מועדונים",diff:"בינוני"},
    {q:"מה שם שחקן שנגן ל-Ajax ו-Barcelona?",a:"פטריק קלויברט",options:["קלארנס זידורף", "פטריק קלויברט", "פרנק ריקארד", "אדגר דאבידס"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה פעמים ריאל מדריד זכה ב-La Liga?",a:"35",options:["28", "32", "35", "40"],cat:"מועדונים",diff:"בינוני"},
    {q:"מי הוא שחקן שנגן ל-Barcelona ו-Juventus?",a:"פאולינהו",options:["פאולינהו", "ברוס", "קוויניה", "נלסון"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה שחקנים כבשו 30+ שערים ב-Champions League?",a:"מספר - ביניהם רונאלדו ומסי",options:["2", "מספר - ביניהם רונאלדו ומסי", "0", "1"],cat:"שיאים",diff:"בינוני"},
    {q:"מי הוא שחקן שנגן ל-Manchester United ו-Real Madrid?",a:"רוד ואן ניסטלרוי",options:["בקהם", "רוד ואן ניסטלרוי", "רונאלדו", "נוייל"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה פעמים ברצלונה זכה ב-Champions League?",a:"5",options:["4", "5", "6", "7"],cat:"מועדונים",diff:"בינוני"},
    {q:"מי ניצח ב-Coppa Italia 2024?",a:"יובנטוס",options:["אינטר", "יובנטוס", "מילאן", "נאפולי"],cat:"מועדונים",diff:"בינוני"},
    {q:"מה שם שחקן שנגן ל-Arsenal ו-Real Madrid?",a:"דני ארסנל",options:["הוז", "דני ארסנל", "גייל", "ריאל"],cat:"שחקנים",diff:"בינוני"},
    {q:"כמה שחקנים ניצחו ה-Champions League עם 3 קבוצות שונות?",a:"0",options:["0", "1", "2", "3"],cat:"שיאים",diff:"בינוני"},
    {q:"מי הוא שחקן שכבש שיא 5 שערים ב-Champions League משחק?",a:"לואיז אדריאנו",options:["מסי", "הולאנד", "לואיז אדריאנו", "בנזמה"],cat:"שיאים",diff:"קשה"},
    {q:"כמה שחקנים ניצחו ה-Champions League בהיותם מאמנים גם?",a:"3",options:["1", "2", "3", "4"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא השחקן עם הכי הרבה בישולים בתולדות ה-Bundesliga?",a:"ריבירה",options:["גרד מולר", "ריבירה", "לבנדובסקי", "בקנבאואר"],cat:"שיאים",diff:"קשה"},
    {q:"כמה שחקנים שיחקו ב-5 גמרי World Cup?",a:"אף אחד - רק ב-4",options:["אף אחד - רק ב-4", "1", "2", "3"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא המאמן שזכה ב-Champions League עם 2 קבוצות שונות?",a:"כמה - ביניהם אנצ'לוטי",options:["זידאן", "פפ גוארדיולה", "כמה - ביניהם אנצ'לוטי", "פרגוסון"],cat:"מאמנים",diff:"קשה"},
    {q:"מה הגיל הצעיר ביותר לשחקן שכבש ב-World Cup Final?",a:"17 - פלה 1958",options:["16", "17 - פלה 1958", "18", "19"],cat:"שיאים",diff:"קשה"},
    {q:"כמה שחקנים ניצחו ה-World Cup ב-2 הזדמנויות שונות?",a:"נדיר - כגון קפלו",options:["0", "נדיר - כגון קפלו", "5", "10"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא שחקן שנגן ב-7 גמרים של Champions League?",a:"לא קיים - שיא הוא 5",options:["מסי", "לא קיים - שיא הוא 5", "רונאלדו", "ריקארד"],cat:"שיאים",diff:"קשה"},
    {q:"כמה שחקנים כבשו ב-6 מונדיאלים שונים?",a:"0",options:["0", "1", "2", "3"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא שחקן שניצח ב-Champions League כשחקן ומאמן עם אותה קבוצה?",a:"פאבלו אימר",options:["פאבלו אימר", "זידאן", "נסקנס", "פיינמן"],cat:"שיאים",diff:"קשה"},
    {q:"כמה שחקנים ניצחו ב-3 World Cups כשחקנים?",a:"0",options:["0", "1", "2", "3"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא שחקן שכבש 91 שערים בשנה קלנדרית אחת?",a:"ליאונל מסי",options:["ליאונל מסי", "גרד מולר", "רונאלדו", "לוטאר מתאוס"],cat:"שיאים",diff:"קשה"},
    {q:"כמה קבוצות זכו ב-Champions League פעמיים ברצף?",a:"מספר - ביניהם ריאל ובייאן",options:["2", "מספר - ביניהם ריאל ובייאן", "0", "1"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא שחקן שניצח ה-Champions League בגיל 18?",a:"סלי מונגאלה",options:["מבאפה", "סלי מונגאלה", "לאמין יאמאל", "בלינגהם"],cat:"שיאים",diff:"קשה"},
    {q:"כמה פעמים ה-Champions League הסתיים ב-0-0 לאחר 90 דקות?",a:"מספר פעמים",options:["0", "מספר פעמים", "1", "2"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא המאמן שזכה ב-6 ליגות שונות?",a:"ג'ואו פלמאה",options:["קרלו אנצ'לוטי", "ג'ואו פלמאה", "פפ גוארדיולה", "מורינייו"],cat:"מאמנים",diff:"קשה"},
    {q:"כמה שחקנים כבשו ב-4 World Cups שונים?",a:"קריסטיאנו רונאלדו",options:["קלוזה", "קריסטיאנו רונאלדו", "פלה", "גארינצ'ה"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא שחקן שניצח ה-Champions League 3 פעמים ברצף כשחקן?",a:"שחקני ריאל 2016-2018",options:["ריקארד", "שחקני ריאל 2016-2018", "בנזמה", "קרוס"],cat:"שיאים",diff:"קשה"},
    {q:"כמה שחקנים כבשו ב-Champions League Final עם 2 קבוצות שונות?",a:"0",options:["0", "1", "2", "3"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא שחקן שניצח ב-World Cup ו-Champions League באותה שנה?",a:"מספר שחקנים",options:["ג'יגס", "מספר שחקנים", "קנס", "ריקן"],cat:"שיאים",diff:"קשה"},
    {q:"כמה שחקנים ניצחו ה-Ballon d'Or 4 פעמים לפחות?",a:"2 - מסי ורונאלדו",options:["1", "2 - מסי ורונאלדו", "3", "4"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא שחקן שכבש בגמר Champions League ל-2 קבוצות שונות?",a:"אין כזה שיא",options:["אין כזה שיא", "דידייה דרוגבה", "בנזמה", "רונאלדו"],cat:"שיאים",diff:"קשה"},
    {q:"כמה פעמים ברזיל ניצחה ב-Maracanazo?",a:"0 - ברזיל הפסידה",options:["0 - ברזיל הפסידה", "1", "2", "3"],cat:"היסטוריה",diff:"קשה"},
    {q:"מי הוא שחקן שכבש ב-10 World Cup games שונים?",a:"כריסטיאנו רונאלדו",options:["מסי", "קלוזה", "כריסטיאנו רונאלדו", "פלה"],cat:"שיאים",diff:"קשה"},
    {q:"כמה שחקנים ניצחו ה-Champions League עם קבוצה מארצם?",a:"מספר שחקנים",options:["0", "מספר שחקנים", "5", "15"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא שחקן שכבש שיא 12 שערים ב-Champions League עונה אחת?",a:"ארלינג הולאנד",options:["מסי", "רונאלדו", "ארלינג הולאנד", "בנזמה"],cat:"שיאים",diff:"קשה"},
    {q:"כמה שחקנים ניצחו ה-World Cup בשתי מדינות שונות?",a:"אסור לשחק ל-2 נבחרות",options:["0", "אסור לשחק ל-2 נבחרות", "1", "2"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא שחקן שכבש ב-5 Champions League Finals?",a:"אף אחד",options:["מסי", "רונאלדו", "אף אחד", "בנזמה"],cat:"שיאים",diff:"קשה"},
    {q:"כמה שחקנים ניצחו ה-Champions League כשחקן בגיל מעל 40?",a:"0",options:["0", "1", "2", "3"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא שחקן שניצח ה-World Cup בפעם השלישית שלו?",a:"פלה 1970",options:["מסי 2022", "גארינצ'ה", "פלה 1970", "זיקו"],cat:"שיאים",diff:"קשה"},
    {q:"כמה שחקנים כבשו ב-Champions League עם קבוצות מ-4 ארצות שונות?",a:"0 ידוע",options:["0 ידוע", "1", "2", "3"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא שחקן שניצח ב-Champions League ו-America's Cup ו-World Cup?",a:"כמה שחקנים - כגון מסי",options:["רונאלדו", "פלה", "כמה שחקנים - כגון מסי", "קאפלו"],cat:"שיאים",diff:"קשה"},
    {q:"כמה שחקנים כבשו ב-4 Champions League גמרים שונים?",a:"0",options:["0", "1", "2", "3"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא מאמן שזכה בכל הגביעים האפשריים?",a:"ג'ואו מוריניו",options:["פפ גוארדיולה", "ג'ואו מוריניו", "אנצ'לוטי", "זידאן"],cat:"מאמנים",diff:"קשה"},
    {q:"כמה שחקנים ניצחו ב-Champions League עם 4 קבוצות שונות?",a:"0",options:["0", "1", "2", "3"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא שחקן שכבש שיא 17 שערים ב-Champions League עונה אחת?",a:"קריסטיאנו רונאלדו 2013/14",options:["מסי", "קריסטיאנו רונאלדו 2013/14", "הולאנד", "בנזמה"],cat:"שיאים",diff:"קשה"},
    {q:"כמה שחקנים ניצחו ה-Champions League ו-World Cup ו-Euros?",a:"מספר שחקנים ספרדים",options:["0", "מספר שחקנים ספרדים", "1", "2"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא שחקן שזכה ב-Champions League ב-5 קבוצות שונות?",a:"0 - לא קיים",options:["גאטוזו", "מארק זוזו", "0 - לא קיים", "ח'אבי"],cat:"שיאים",diff:"קשה"},
    {q:"כמה שחקנים ניצחו ב-World Cup ו-Champions League ב-3 עשורים שונים?",a:"0",options:["0", "1", "2", "3"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא שחקן שכבש ב-100 משחקי Champions League?",a:"קריסטיאנו רונאלדו",options:["מסי", "קריסטיאנו רונאלדו", "בנזמה", "הולאנד"],cat:"שיאים",diff:"קשה"},
    {q:"כמה שחקנים ניצחו ה-Champions League עם קבוצה שלא מארצם?",a:"רוב הזוכים",options:["0", "מעטים", "רוב הזוכים", "כולם"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא שחקן שניצח ה-World Cup בגיל הכי מבוגר?",a:"דינו זוף 1982",options:["פלה", "דינו זוף 1982", "מסי", "לוטאר"],cat:"שיאים",diff:"קשה"},
    {q:"כמה שחקנים ניצחו ה-Champions League ב-4 עשורים שונים?",a:"0",options:["0", "1", "2", "3"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא שחקן שכבש שיא ב-La Liga בגיל הצעיר ביותר?",a:"לאמין יאמאל",options:["מסי", "פדרי", "לאמין יאמאל", "גאבי"],cat:"שיאים",diff:"קשה"},
    {q:"כמה שחקנים ניצחו ב-Champions League ו-Copa Libertadores?",a:"0",options:["0", "1", "2", "3"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא שחקן שזכה ב-Champions League עם הקבוצה הכחולה ביותר?",a:"ניקולאס אנלקה עם צ'לסי",options:["ניקולאס אנלקה עם צ'לסי", "למפרד", "דרוגבה", "ג'ון טרי"],cat:"שיאים",diff:"קשה"},
    {q:"כמה שחקנים ניצחו ה-World Cup ב-3 הזדמנויות כשחקנים?",a:"0",options:["0", "פלה", "1", "2"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא שחקן שניצח ה-Champions League בגיל הצעיר ביותר?",a:"שחקן ב-1986",options:["בלינגהם", "מבאפה", "שחקן ב-1986", "לאמין יאמאל"],cat:"שיאים",diff:"קשה"},
    {q:"כמה שחקנים זכו ב-Champions League ו-World Cup ביחד בשנה?",a:"קרה ב-1974 ו-1982",options:["0", "קרה ב-1974 ו-1982", "לעיתים רחוקות", "כל שנה"],cat:"שיאים",diff:"קשה"},
    {q:"מי הוא שחקן שכבש שיא ב-Copa America?",a:"ניאורו",options:["מסי", "ניאורו", "פלה", "ח'נטה"],cat:"שיאים",diff:"קשה"},
    {q:"כמה פעמים ניצחה ארגנטינה ב-World Cup?",a:"3",options:["2", "3", "4", "1"],cat:"נבחרות",diff:"קשה"},
  ];
...
const DAILY_Q = {
  "Spain": [
    { q:"כמה פעמים זכתה ספרד בגביע העולם?", a:"פעם אחת", opts:["פעמיים", "שלוש פעמים", "אף פעם"], diff:"קל" },
    { q:"באיזו שנה זכתה ספרד בגביע העולם?", a:"2010", opts:["2006", "2014", "2002"], diff:"קל" },
    { q:"מה עיר הבירה של ספרד?", a:"מדריד", opts:["ברצלונה", "סביליה", "ולנסיה"], diff:"קל" },
    { q:"כמה תושבים יש בספרד (בערך)?", a:"47 מיליון", opts:["30 מיליון", "60 מיליון", "80 מיליון"], diff:"בינוני" },
    { q:"מי כוכב נבחרת ספרד הנוכחי?", a:"למין ימאל", opts:["פדרי", "ניקו וויליאמס", "מוראטה"], diff:"קל" },
    { q:"באיזה ליגה משחק ריאל מדריד?", a:"לה ליגה", opts:["פרמייר ליג", "בונדסליגה", "סרייה א"], diff:"קל" },
    { q:"מה המטבע של ספרד?", a:"אירו", opts:["פסטה", "קרונה", "פאונד"], diff:"קל" },
    { q:"כמה פעמים זכתה ספרד ביורו?", a:"4 פעמים", opts:["פעמיים", "3 פעמים", "פעם אחת"], diff:"בינוני" },
  ],
  "Cabo Verde": [
    { q:"איפה ממוקמת כף ורדה?", a:"האוקיינוס האטלנטי", opts:["הים התיכון", "האוקיינוס ההודי", "הים הקריבי"], diff:"בינוני" },
    { q:"מה עיר הבירה של כף ורדה?", a:"פראיה", opts:["מינדלו", "סנטה מריה", "אסלגה"], diff:"קשה" },
    { q:"כמה תושבים יש בכף ורדה (בערך)?", a:"600 אלף", opts:["2 מיליון", "5 מיליון", "100 אלף"], diff:"בינוני" },
    { q:"לאיזו מדינה הייתה כף ורדה מושבה?", a:"פורטוגל", opts:["ספרד", "צרפת", "בריטניה"], diff:"בינוני" },
    { q:"מה שפת הרשמי של כף ורדה?", a:"פורטוגזית", opts:["ספרדית", "צרפתית", "אנגלית"], diff:"קל" },
    { q:"מה אחת הנקודות המפורסמות של כף ורדה?", a:"חופי הגלישה", opts:["הרי הגעש", "ג'ונגל", "הפירמידות"], diff:"בינוני" },
    { q:"מה מספר האיים של כף ורדה?", a:"10 איים", opts:["5 איים", "3 איים", "20 איים"], diff:"קשה" },
    { q:"מי כוכב נבחרת כף ורדה?", a:"גרי רודריגס", opts:["ז'מירו מונטיירו", "ריאן מנדס", "דרוי דוארטה"], diff:"קשה" },
  ],
  "Belgium": [
    { q:"מה עיר הבירה של בלגיה?", a:"בריסל", opts:["אנטוורפן", "גנט", "ליאז'"], diff:"קל" },
    { q:"מי הכוכב הגדול של בלגיה בדור הזהב?", a:"קווין דה ברויינה", opts:["רומלו לוקאקו", "תיבו קורטואה", "ז'רמי דוקו"], diff:"קל" },
    { q:"מה ההישג הגדול של בלגיה במונדיאל?", a:"מקום 3 (2018)", opts:["גמר (2014)", "מקום 4 (2022)", "ניצחון (1986)"], diff:"בינוני" },
    { q:"כמה תושבים יש בבלגיה (בערך)?", a:"11 מיליון", opts:["5 מיליון", "20 מיליון", "30 מיליון"], diff:"בינוני" },
    { q:"בלגיה גובלת עם כמה מדינות?", a:"4", opts:["2", "3", "5"], diff:"קשה" },
    { q:"מה המשקה המפורסם של בלגיה?", a:"בירה", opts:["וויסקי", "וודקה", "ראם"], diff:"קל" },
    { q:"באיזו ליגה משחק Club Brugge?", a:"ליגת פרו בלגיה", opts:["ליגה 1", "בונדסליגה", "ארדיביזי"], diff:"בינוני" },
    { q:"מה השפות הרשמיות של בלגיה?", a:"הולנדית, צרפתית, גרמנית", opts:["צרפתית בלבד", "הולנדית וצרפתית", "אנגלית וצרפתית"], diff:"בינוני" },
  ],
  "Egypt": [
    { q:"מה עיר הבירה של מצרים?", a:"קהיר", opts:["אלכסנדריה", "לוקסור", "אסואן"], diff:"קל" },
    { q:"מי כוכב נבחרת מצרים?", a:"מוחמד סלאח", opts:["מוסטפא מוחמד", "מחמוד תרזיגוי", "עמר קמל"], diff:"קל" },
    { q:"כמה פעמים זכתה מצרים בגביע אפריקה?", a:"7 פעמים", opts:["3 פעמים", "5 פעמים", "10 פעמים"], diff:"בינוני" },
    { q:"כמה תושבים יש במצרים (בערך)?", a:"105 מיליון", opts:["50 מיליון", "80 מיליון", "150 מיליון"], diff:"בינוני" },
    { q:"על איזה נהר יושבת מצרים?", a:"הנילוס", opts:["הירדן", "הקונגו", "הניז'ר"], diff:"קל" },
    { q:"מה אחת מהפלאות העתיקות שנמצאת במצרים?", a:"הפירמידות של גיזה", opts:["קולוסיאום", "מגדל בבל", "אנגקור וואט"], diff:"קל" },
    { q:"באיזו שנה שיחקה מצרים לאחרונה במונדיאל?", a:"1990", opts:["2006", "2002", "1998"], diff:"קשה" },
    { q:"מה התעלה הימית המפורסמת שמחברת בין הים התיכון לים האדום?", a:"תעלת סואץ", opts:["תעלת פנמה", "תעלת קיל", "תעלת מנצ'סטר"], diff:"קל" },
  ],
  "Saudi Arabia": [
    { q:"מה עיר הבירה של ערב הסעודית?", a:"ריאד", opts:["ג'דה", "מכה", "מדינה"], diff:"קל" },
    { q:"מי כוכב נבחרת ערב הסעודית?", a:"סאלם אל-דוסרי", opts:["פירס אל-בוריקאן", "עבדאללה אל-חמדאן", "מוחמד אל-עוואיס"], diff:"בינוני" },
    { q:"ב-2022 ערב הסעודית ניצחה איזו נבחרת?", a:"ארגנטינה", opts:["ברזיל", "צרפת", "גרמניה"], diff:"קל" },
    { q:"כמה תושבים יש בערב הסעודית (בערך)?", a:"36 מיליון", opts:["10 מיליון", "60 מיליון", "100 מיליון"], diff:"בינוני" },
    { q:"מה המשאב הטבעי המפורסם של ערב הסעודית?", a:"נפט", opts:["יהלומים", "זהב", "גז טבעי"], diff:"קל" },
    { q:"איזו ליגה נקנתה בכסף סעודי עם שחקנים כמו רונאלדו?", a:"הליגה הסעודית לכדורגל", opts:["ליגה אמירוייתית", "ליגה קטרית", "ליגה כוויתית"], diff:"קל" },
    { q:"מה הדת הרשמית של ערב הסעודית?", a:"אסלאם", opts:["נצרות", "יהדות", "בודהיזם"], diff:"קל" },
    { q:"באיזו יבשת נמצאת ערב הסעודית?", a:"אסיה", opts:["אפריקה", "אירופה", "אמריקה"], diff:"קל" },
  ],
  "Uruguay": [
    { q:"כמה פעמים זכתה אורוגוואי בגביע העולם?", a:"פעמיים", opts:["פעם אחת", "3 פעמים", "אף פעם"], diff:"בינוני" },
    { q:"מה עיר הבירה של אורוגוואי?", a:"מונטווידאו", opts:["בואנוס איירס", "לימה", "אסונסיון"], diff:"בינוני" },
    { q:"מי כוכב נבחרת אורוגוואי?", a:"דארווין נונייס", opts:["לואיס סוארס", "פדריקו ולברדה", "רונאלד ארוחו"], diff:"קל" },
    { q:"כמה תושבים יש באורוגוואי (בערך)?", a:"3.5 מיליון", opts:["10 מיליון", "20 מיליון", "1 מיליון"], diff:"בינוני" },
    { q:"באיזו יבשת נמצאת אורוגוואי?", a:"דרום אמריקה", opts:["אמריקה המרכזית", "צפון אמריקה", "אירופה"], diff:"קל" },
    { q:"לואיס סוארס מפורסם בנשיכה של מי?", a:"קיילני צ'יאליני", opts:["מסי", "רונאלדו", "ניימאר"], diff:"בינוני" },
    { q:"אורוגוואי גובלת עם אילו מדינות?", a:"ברזיל וארגנטינה", opts:["צ'ילה וברזיל", "פרגוואי וברזיל", "בוליביה וארגנטינה"], diff:"בינוני" },
    { q:"איזה אצטדיון מפורסם נמצא במונטווידאו?", a:"אסטדיו סנטנריו", opts:["מרקנה", "סנטיאגו ברנביאו", "קמפ נואו"], diff:"קשה" },
  ],
  "Iran": [
    { q:"מה עיר הבירה של איראן?", a:"טהרן", opts:["ישפהאן", "שיראז", "תבריז"], diff:"קל" },
    { q:"מי כוכב נבחרת איראן?", a:"מהדי טארמי", opts:["סרדאר אזמון", "עלי גולי-זאדה", "עליריזה ג'חאנבחש"], diff:"בינוני" },
    { q:"כמה תושבים יש באיראן (בערך)?", a:"87 מיליון", opts:["40 מיליון", "60 מיליון", "120 מיליון"], diff:"בינוני" },
    { q:"מה השפה הרשמית של איראן?", a:"פרסית", opts:["ערבית", "טורקית", "כורדית"], diff:"בינוני" },
    { q:"באיזו יבשת נמצאת איראן?", a:"אסיה", opts:["אפריקה", "אירופה", "אוקיאניה"], diff:"קל" },
    { q:"איראן ידועה בייצוא של מה?", a:"נפט וגז טבעי", opts:["יהלומים", "חיטה", "עץ"], diff:"קל" },
    { q:"מה הכינוי של נבחרת איראן?", a:"Team Melli", opts:["Persian Lions", "Eagles", "Desert Hawks"], diff:"קשה" },
    { q:"כמה פעמים השתתפה איראן במונדיאל?", a:"6 פעמים", opts:["3 פעמים", "4 פעמים", "8 פעמים"], diff:"בינוני" },
  ],
  "New Zealand": [
    { q:"מה עיר הבירה של ניו זילנד?", a:"ווליגטון", opts:["אוקלנד", "כרייסטצ'רץ", "הובארט"], diff:"בינוני" },
    { q:"מה הכינוי של נבחרת ניו זילנד?", a:"All Whites", opts:["Kiwis", "Black Ferns", "Silver Ferns"], diff:"בינוני" },
    { q:"באיזו יבשת נמצאת ניו זילנד?", a:"אוקיאניה", opts:["אסיה", "אוסטרליה", "אנטארקטיקה"], diff:"קל" },
    { q:"כמה תושבים יש בניו זילנד (בערך)?", a:"5 מיליון", opts:["2 מיליון", "10 מיליון", "20 מיליון"], diff:"בינוני" },
    { q:"מה הבעל החיים הסמלי של ניו זילנד?", a:"קיווי", opts:["קנגורו", "קואלה", "פינגווין"], diff:"קל" },
    { q:"ניו זילנד ידועה בסדרת סרטים של מה?", a:"שר הטבעות", opts:["הארי פוטר", "אווטאר", "מלחמת הכוכבים"], diff:"קל" },
    { q:"מי שחקן הכדורגל המפורסם מניו זילנד?", a:"כריס וד", opts:["ווינסטון ריד", "אוליבר סייל", "מייקל בוקסל"], diff:"קשה" },
    { q:"כמה איים עיקריים יש בניו זילנד?", a:"2", opts:["1", "3", "4"], diff:"קל" },
  ],
  "Mexico": [
    { q:"כמה פעמים ארחה מקסיקו את גביע העולם?", a:"פעמיים", opts:["פעם אחת", "3 פעמים", "אף פעם"], diff:"בינוני" },
    { q:"מה עיר הבירה של מקסיקו?", a:"מקסיקו סיטי", opts:["גוודלחרה", "מונטריי", "קנקון"], diff:"קל" },
    { q:"מי כוכב נבחרת מקסיקו?", a:"סנטיאגו חימנז", opts:["הירבינג לוזאנו", "ראול חימנז", "אדסון אלברז"], diff:"בינוני" },
    { q:"כמה תושבים יש במקסיקו (בערך)?", a:"130 מיליון", opts:["50 מיליון", "80 מיליון", "200 מיליון"], diff:"בינוני" },
    { q:"מה ה-16 consecutive שיא מקסיקו עם?", a:"הגעה לשמינית הגמר", opts:["הגעה לרבע גמר", "זכייה בגביע", "הגעה לגמר"], diff:"קשה" },
    { q:"מה המאכל המפורסם ממקסיקו?", a:"טאקו", opts:["בוריטו", "נאצ'וס", "קסדיה"], diff:"קל" },
    { q:"מקסיקו גובלת עם ארה״ב בצפון ועם מה בדרום?", a:"גואטמלה ובליז", opts:["קולומביה וקוסטה ריקה", "ונצואלה וקובה", "אל סלוודור בלבד"], diff:"קשה" },
    { q:"באיזו קונפדרציה משחקת מקסיקו?", a:"CONCACAF", opts:["CONMEBOL", "UEFA", "CAF"], diff:"בינוני" },
  ],
  "South Africa": [
    { q:"כמה ערי בירה יש לדרום אפריקה?", a:"3", opts:["1", "2", "4"], diff:"קשה" },
    { q:"מי ארח את מונדיאל 2010?", a:"דרום אפריקה", opts:["ניגריה", "קניה", "מרוקו"], diff:"קל" },
    { q:"מה מנגן בכל משחק מ-2010?", a:"ווּווּזלה", opts:["תוף", "חליל", "כינור"], diff:"קל" },
    { q:"כמה תושבים יש בדרום אפריקה (בערך)?", a:"60 מיליון", opts:["30 מיליון", "80 מיליון", "120 מיליון"], diff:"בינוני" },
    { q:"מי הנשיא המפורסם של דרום אפריקה מהמאה ה-20?", a:"נלסון מנדלה", opts:["דזמונד טוטו", "אברהם לינקולן", "גנדי"], diff:"קל" },
    { q:"מי כוכב נבחרת דרום אפריקה?", a:"פרסי טאו", opts:["ליל פוסטר", "רונווין וויליאמס", "אביידנס מאקגופה"], diff:"בינוני" },
    { q:"מה היהלום המפורסם שנמצא בדרום אפריקה?", a:"הקולינאן", opts:["קוה-אי-נור", "הופ", "ריג'נט"], diff:"קשה" },
    { q:"מה האוקיינוס שגובל בדרום אפריקה מדרום?", a:"שניהם — אטלנטי והודי", opts:["האטלנטי בלבד", "ההודי בלבד", "הפסיפי"], diff:"בינוני" },
  ],
  "South Korea": [
    { q:"מה עיר הבירה של דרום קוריאה?", a:"סיאול", opts:["בוסאן", "דאגו", "אינצ'ון"], diff:"קל" },
    { q:"מי הכוכב של נבחרת דרום קוריאה?", a:"סון הה-מין", opts:["לי קאנג-אין", "הוואנג הי-צ'אן", "קים מין-ג'יי"], diff:"קל" },
    { q:"מה ההישג הגדול ביותר של קוריאה במונדיאל?", a:"מקום 4 (2002)", opts:["מקום 3 (2010)", "גמר (2002)", "שמינית (2018)"], diff:"בינוני" },
    { q:"מה מותג הרכב המפורסם מקוריאה?", a:"יונדאי / קיה", opts:["טויוטה", "הונדה", "מיצובישי"], diff:"קל" },
    { q:"כמה תושבים יש בדרום קוריאה (בערך)?", a:"52 מיליון", opts:["20 מיליון", "80 מיליון", "100 מיליון"], diff:"בינוני" },
    { q:"מה הסדרה הקוריאנית המפורסמת בנטפליקס?", a:"Squid Game", opts:["Parasite", "Kingdom", "All of Us Are Dead"], diff:"קל" },
    { q:"מה סגנון המוזיקה הפופולרי מקוריאה?", a:"K-Pop", opts:["J-Pop", "C-Pop", "T-Pop"], diff:"קל" },
    { q:"באיזו קונפדרציה משחקת קוריאה?", a:"AFC", opts:["CONCACAF", "UEFA", "CAF"], diff:"בינוני" },
  ],
  "Czechia": [
    { q:"מה עיר הבירה של צ'כיה?", a:"פראג", opts:["ברנו", "אוסטרבה", "פילזן"], diff:"קל" },
    { q:"מי כוכב נבחרת צ'כיה?", a:"פטריק שיק", opts:["אדם הלוז'ק", "תומס סוצ'ק", "ולדימיר קופ'אל"], diff:"בינוני" },
    { q:"מה הבירה של צ'כיה ידועה בה?", a:"בירה", opts:["יין", "וויסקי", "וודקה"], diff:"קל" },
    { q:"כמה תושבים יש בצ'כיה (בערך)?", a:"10 מיליון", opts:["5 מיליון", "20 מיליון", "30 מיליון"], diff:"בינוני" },
    { q:"מה הכינוי של נבחרת צ'כיה?", a:"הלביאים", opts:["הנשרים", "הדובים", "הצבאים"], diff:"קשה" },
    { q:"באיזו שנה ניצחה צ'כוסלובקיה ביורו?", a:"1976", opts:["1988", "1992", "1968"], diff:"קשה" },
    { q:"מה המכונית המפורסמת שמיוצרת בצ'כיה?", a:"שקודה", opts:["סיאט", "דאצ'יה", "אלפא רומאו"], diff:"בינוני" },
    { q:"צ'כיה ניצחה ב-EURO 1996 בגמר נגד מי?", a:"הגיעה לגמר אבל הפסידה לגרמניה", opts:["ניצחה את גרמניה", "הפסידה לצרפת", "ניצחה את הולנד"], diff:"קשה" },
  ],
  "Canada": [
    { q:"מה עיר הבירה של קנדה?", a:"אוטווה", opts:["טורונטו", "ונקובר", "מונטריאול"], diff:"בינוני" },
    { q:"מי כוכב נבחרת קנדה?", a:"אלפונסו דייויס", opts:["ג'ונתן דייויד", "סטיבן אויסטאקיו", "טאז'ון בוקנן"], diff:"קל" },
    { q:"קנדה מארחת את מונדיאל 2026 יחד עם אילו מדינות?", a:"ארה״ב ומקסיקו", opts:["ארה״ב ובריטניה", "מקסיקו ואמריקה המרכזית", "ארה״ב בלבד"], diff:"קל" },
    { q:"כמה תושבים יש בקנדה (בערך)?", a:"38 מיליון", opts:["20 מיליון", "60 מיליון", "100 מיליון"], diff:"בינוני" },
    { q:"מה העלה שמופיע על דגל קנדה?", a:"מייפל", opts:["אלון", "אורן", "לבנה"], diff:"קל" },
    { q:"מה הספורט הלאומי הרשמי של קנדה?", a:"הוקי קרח ולאקרוס", opts:["כדורגל", "בייסבול", "כדורסל"], diff:"בינוני" },
    { q:"באיזו קונפדרציה משחקת קנדה?", a:"CONCACAF", opts:["CONMEBOL", "UEFA", "AFC"], diff:"בינוני" },
    { q:"מה שתי השפות הרשמיות של קנדה?", a:"אנגלית וצרפתית", opts:["אנגלית ואינואיט", "צרפתית וספרדית", "אנגלית בלבד"], diff:"קל" },
  ],
  "Bosnia": [
    { q:"מה עיר הבירה של בוסניה?", a:"סרייבו", opts:["בניה לוקה", "מוסטאר", "טוזלה"], diff:"בינוני" },
    { q:"מי כוכב נבחרת בוסניה?", a:"ארמין דמירוביץ'", opts:["אדין ויסצ'ה", "אדין ג'קו", "מירלם פיאניץ'"], diff:"בינוני" },
    { q:"באיזו שנה השתתפה בוסניה לראשונה במונדיאל?", a:"2014", opts:["2010", "2018", "2006"], diff:"בינוני" },
    { q:"כמה תושבים יש בבוסניה (בערך)?", a:"3.2 מיליון", opts:["1 מיליון", "8 מיליון", "15 מיליון"], diff:"קשה" },
    { q:"מה הגשר המפורסם בבוסניה?", a:"הגשר הישן של מוסטאר", opts:["גשר לונדון", "גשר ברוקלין", "גשר סידני"], diff:"בינוני" },
    { q:"בוסניה הייתה חלק ממה עד 1992?", a:"יוגוסלביה", opts:["ברית המועצות", "צ'כוסלובקיה", "אוסטרו-הונגריה"], diff:"בינוני" },
    { q:"מירלם פיאניץ' שיחק לאיזה מועדון?", a:"יובנטוס וברצלונה", opts:["מנצ'סטר יונייטד", "ריאל מדריד", "בייארן מינכן"], diff:"בינוני" },
    { q:"מה הנהר שזורם דרך סרייבו?", a:"מיליאצ'קה", opts:["דנובה", "סאווה", "דרינה"], diff:"קשה" },
  ],
  "Qatar": [
    { q:"קטאר ארחה את מונדיאל באיזו שנה?", a:"2022", opts:["2018", "2014", "2026"], diff:"קל" },
    { q:"מה עיר הבירה של קטאר?", a:"דוחא", opts:["אל-חוור", "לוסיל", "אל-ראיאן"], diff:"קל" },
    { q:"מי כוכב נבחרת קטאר?", a:"אקרם עפיף", opts:["אלמועז עלי", "מוחמד מונטרי", "הסן אל-היידוס"], diff:"בינוני" },
    { q:"כמה תושבים יש בקטאר (בערך)?", a:"2.9 מיליון", opts:["500 אלף", "10 מיליון", "20 מיליון"], diff:"בינוני" },
    { q:"מה ערוץ הספורט המפורסם ממקטאר?", a:"beIN Sports", opts:["Sky Sports", "ESPN", "DAZN"], diff:"בינוני" },
    { q:"קטאר ידועה בייצוא של מה?", a:"גז טבעי", opts:["נפט בלבד", "יהלומים", "זהב"], diff:"בינוני" },
    { q:"מה המועדון הכי מפורסם בקטאר?", a:"אל-סד", opts:["אל-הילאל", "אל-אהלי", "אל-ריאן"], diff:"קשה" },
    { q:"קטאר נמצאת באיזו חצי אי?", a:"חצי האי הערבי", opts:["חצי האי סיני", "חצי האי הבלקן", "חצי האי איברי"], diff:"בינוני" },
  ],
  "Switzerland": [
    { q:"מה עיר הבירה של שוויץ?", a:"ברן", opts:["ציריך", "ז'נבה", "בזל"], diff:"בינוני" },
    { q:"מי כוכב נבחרת שוויץ?", a:"גרניט ז'אקה", opts:["רמו פרוילר", "ברל אמבולו", "זקי אמדוני"], diff:"בינוני" },
    { q:"כמה שפות רשמיות יש בשוויץ?", a:"4", opts:["1", "2", "3"], diff:"בינוני" },
    { q:"כמה תושבים יש בשוויץ (בערך)?", a:"8.7 מיליון", opts:["3 מיליון", "20 מיליון", "40 מיליון"], diff:"בינוני" },
    { q:"מה מפורסמת שוויץ בייצור?", a:"שוקולד ושעונים", opts:["מכוניות ואלקטרוניקה", "נפט וגז", "אופנה ובישום"], diff:"קל" },
    { q:"איפה נמצא המטה של FIFA?", a:"ציריך, שוויץ", opts:["ז'נבה, שוויץ", "ברסל, בלגיה", "לוזאן, שוויץ"], diff:"בינוני" },
    { q:"שוויץ ידועה בניטרליות מאז?", a:"1815", opts:["1945", "1918", "1648"], diff:"קשה" },
    { q:"איזה הר גבוה נמצא בשוויץ?", a:"מטרהורן", opts:["מון בלאן", "אייגר", "יונגפראו"], diff:"בינוני" },
  ],
  "Brazil": [
    { q:"כמה פעמים זכתה ברזיל בגביע העולם?", a:"5 פעמים", opts:["3 פעמים", "4 פעמים", "6 פעמים"], diff:"קל" },
    { q:"מה עיר הבירה של ברזיל?", a:"ברזיליה", opts:["ריו דה ז'נרו", "סאו פאולו", "סלוודור"], diff:"בינוני" },
    { q:"מי כוכב נבחרת ברזיל?", a:"וויניסיוס ג'וניור", opts:["רודריגו", "ראפינה", "לוקאס פאקטה"], diff:"קל" },
    { q:"כמה תושבים יש בברזיל (בערך)?", a:"215 מיליון", opts:["100 מיליון", "150 מיליון", "300 מיליון"], diff:"בינוני" },
    { q:"מה נהר הגדול שזורם בברזיל?", a:"האמזונס", opts:["הנילוס", "המיסיסיפי", "האוריינוקו"], diff:"קל" },
    { q:"מה הספורט הכי פופולרי בברזיל?", a:"כדורגל", opts:["כדורגל חוף", "ג'יו ג'יטסו", "כדורעף"], diff:"קל" },
    { q:"מה הכינוי של נבחרת ברזיל?", a:"סלסאו", opts:["ורדה-אמרלה", "קנריה", "פנטרה"], diff:"בינוני" },
    { q:"ברזיל היא המדינה הגדולה ביבשת:", a:"דרום אמריקה", opts:["צפון אמריקה", "אפריקה", "אסיה"], diff:"קל" },
  ],
  "Morocco": [
    { q:"ב-2022 עד איפה הגיעה מרוקו?", a:"חצי גמר", opts:["רבע גמר", "גמר", "שמינית גמר"], diff:"קל" },
    { q:"מה עיר הבירה של מרוקו?", a:"רבאט", opts:["קזבלנקה", "מראקש", "פס"], diff:"בינוני" },
    { q:"מי כוכב נבחרת מרוקו?", a:"חכים זיאש", opts:["יוסף אן-נסירי", "אשרף חכימי", "סופיאן אמראבאט"], diff:"קל" },
    { q:"כמה תושבים יש במרוקו (בערך)?", a:"37 מיליון", opts:["15 מיליון", "60 מיליון", "100 מיליון"], diff:"בינוני" },
    { q:"מרוקו נמצאת באיזו יבשת?", a:"אפריקה", opts:["אסיה", "אירופה", "המזרח התיכון"], diff:"קל" },
    { q:"מה הים שגובל במרוקו מהצפון?", a:"הים התיכון", opts:["הים האדום", "האוקיינוס ההודי", "הים הקסבי"], diff:"בינוני" },
    { q:"מה השפה הרשמית של מרוקו?", a:"ערבית ותמזיגת", opts:["צרפתית", "ספרדית", "ערבית בלבד"], diff:"קשה" },
    { q:"מה המדבר שנמצא במרוקו?", a:"הסהרה", opts:["גובי", "קלהארי", "נגב"], diff:"קל" },
  ],
  "Haiti": [
    { q:"מה עיר הבירה של האיטי?", a:"פורטו-פרנס", opts:["קאפ-האיטיין", "גונאיב", "לס קייס"], diff:"בינוני" },
    { q:"האיטי חולקת את האי עם מה?", a:"הרפובליקה הדומיניקנית", opts:["קובה", "פורטו ריקו", "ג'מייקה"], diff:"בינוני" },
    { q:"כמה תושבים יש באיטי (בערך)?", a:"11 מיליון", opts:["3 מיליון", "20 מיליון", "50 מיליון"], diff:"בינוני" },
    { q:"האיטי הייתה מושבה של מה?", a:"צרפת", opts:["ספרד", "ארה״ב", "בריטניה"], diff:"בינוני" },
    { q:"מה השפה הרשמית של האיטי?", a:"צרפתית וקריאולית", opts:["ספרדית", "אנגלית", "פורטוגזית"], diff:"בינוני" },
    { q:"האיטי הייתה הרפובליקה הראשונה שהשיגה עצמאות:", a:"על ידי מרד עבדים", opts:["דרך בחירות", "דרך הסכם שלום", "דרך מלחמת עצמאות רגילה"], diff:"קשה" },
    { q:"מה אחד האתגרים הגדולים של האיטי?", a:"רעידות אדמה ועוני", opts:["מלחמות גבולות", "מחסור בנפט", "שיטפונות בלבד"], diff:"קל" },
    { q:"מי כוכב נבחרת האיטי?", a:"דאקנס נזון", opts:["ג'וני פלאסיד", "קרבנס בלפור", "פרנצ'די פיירו"], diff:"קשה" },
  ],
  "Scotland": [
    { q:"מה עיר הבירה של סקוטלנד?", a:"אדינבורו", opts:["גלאזגו", "אברדין", "דאנדי"], diff:"קל" },
    { q:"מי כוכב נבחרת סקוטלנד?", a:"סקוט מקתומיני", opts:["אנדרו רובינסון", "ג'ון מקגין", "בילי גילמור"], diff:"בינוני" },
    { q:"מה המשקה הלאומי של סקוטלנד?", a:"וויסקי סקוצ'י", opts:["בירה", "ג'ין", "רום"], diff:"קל" },
    { q:"כמה תושבים יש בסקוטלנד (בערך)?", a:"5.5 מיליון", opts:["2 מיליון", "10 מיליון", "20 מיליון"], diff:"בינוני" },
    { q:"מה הכלי המוזיקלי המסורתי של סקוטלנד?", a:"חצוצרת השק (Bagpipes)", opts:["כינור", "עוד", "הרמוניקה"], diff:"קל" },
    { q:"מה המפלצת המפורסמת מסקוטלנד?", a:"נס לוך", opts:["ביגפוט", "יטי", "גורגון"], diff:"קל" },
    { q:"סקוטלנד היא חלק ממה?", a:"הממלכה המאוחדת", opts:["אירלנד", "ברית האירופית", "מדינה עצמאית"], diff:"קל" },
    { q:"בסקוטלנד נולד מי שהמציא את הטלפון?", a:"אלכסנדר גרהם בל", opts:["תומאס אדיסון", "ניקולה טסלה", "גוגלייאלמו מרקוני"], diff:"בינוני" },
  ],
  "USA": [
    { q:"מה עיר הבירה של ארה״ב?", a:"וושינגטון DC", opts:["ניו יורק", "לוס אנג'לס", "שיקגו"], diff:"קל" },
    { q:"מי כוכב נבחרת ארה״ב?", a:"כריסטיאן פוליסיץ'", opts:["וסטון מקני", "יונוס מוסה", "טיילר אדמס"], diff:"קל" },
    { q:"ארה״ב הגיעה לחצי גמר של מונדיאל באיזו שנה?", a:"1930", opts:["1950", "1994", "2002"], diff:"קשה" },
    { q:"כמה תושבים יש בארה״ב (בערך)?", a:"335 מיליון", opts:["200 מיליון", "400 מיליון", "500 מיליון"], diff:"בינוני" },
    { q:"כמה כוכבים יש על דגל ארה״ב?", a:"50", opts:["48", "52", "45"], diff:"קל" },
    { q:"מה המשמעות של MLS (ליגת הכדורגל האמריקאית)?", a:"Major League Soccer", opts:["Major League Sports", "Masters League Soccer", "Mega League Soccer"], diff:"קל" },
    { q:"איזה מגרש בניו יורק קרוי על שם אגדת כדורגל?", a:"אין — נקרא Red Bull Arena", opts:["Pele Stadium", "Beckham Park", "Maradona Field"], diff:"קשה" },
    { q:"ארה״ב ארחה את מונדיאל 1994, כמה צופים הגיעו סה״כ?", a:"3.5 מיליון — שיא", opts:["1 מיליון", "2 מיליון", "5 מיליון"], diff:"קשה" },
  ],
  "Paraguay": [
    { q:"מה עיר הבירה של פרגוואי?", a:"אסונסיון", opts:["סיאודד דל אסטה", "אנקרנסיון", "קונסנסיון"], diff:"בינוני" },
    { q:"מי כוכב נבחרת פרגוואי?", a:"מיגל אלמירון", opts:["אנטוניו סנבריה", "גוסטבו גומז", "ריצ'רד סנצ'ז"], diff:"בינוני" },
    { q:"כמה תושבים יש בפרגוואי (בערך)?", a:"7 מיליון", opts:["2 מיליון", "15 מיליון", "30 מיליון"], diff:"קשה" },
    { q:"פרגוואי היא אחת משתי מדינות ב-S. America ללא:", a:"גישה לים", opts:["גבול עם ברזיל", "נהרות", "הרים"], diff:"בינוני" },
    { q:"מה השפות הרשמיות של פרגוואי?", a:"ספרדית וגוארני", opts:["ספרדית בלבד", "פורטוגזית וספרדית", "גוארני בלבד"], diff:"קשה" },
    { q:"פרגוואי ידועה בייצור:", a:"חשמל מהידרואלקטרי", opts:["נפט", "זהב", "יהלומים"], diff:"קשה" },
    { q:"ב-2010 ישראל מי הגיעה פרגוואי?", a:"רבע גמר", opts:["שמינית גמר", "חצי גמר", "גמר"], diff:"בינוני" },
    { q:"מה הנהר הגדול שגובל בפרגוואי?", a:"הפרגוואי והפרנה", opts:["האמזונס", "הלה פלטה", "האורינוקו"], diff:"קשה" },
  ],
  "Australia": [
    { q:"מה עיר הבירה של אוסטרליה?", a:"קנברה", opts:["סידני", "מלבורן", "ברזביין"], diff:"בינוני" },
    { q:"מי כוכב נבחרת אוסטרליה?", a:"מאטיו ראיאן", opts:["מיטשל דוק", "מאט לקי", "ריילי מקגרי"], diff:"קשה" },
    { q:"אוסטרליה הגיעה לחצי גמר מונדיאל:", a:"2006", opts:["2010", "2014", "2022"], diff:"בינוני" },
    { q:"מה בעל החיים הסמלי של אוסטרליה?", a:"קנגורו וארנבת השלג", opts:["קואלה בלבד", "קנגורו בלבד", "קוקאבורה"], diff:"בינוני" },
    { q:"כמה תושבים יש באוסטרליה (בערך)?", a:"26 מיליון", opts:["10 מיליון", "50 מיליון", "100 מיליון"], diff:"בינוני" },
    { q:"מה האוקיינוס שמזרח לאוסטרליה?", a:"הפסיפי", opts:["ההודי", "האטלנטי", "הארקטי"], diff:"קל" },
    { q:"מה הכינוי של נבחרת אוסטרליה?", a:"Socceroos", opts:["Kangaroos", "Wallabies", "Matildas"], diff:"בינוני" },
    { q:"מה השונית המפורסמת שנמצאת ליד אוסטרליה?", a:"שונית המחסום הגדולה", opts:["שונית בליז", "שונית הפניקס", "שונית מלדיב"], diff:"קל" },
  ],
  "Türkiye": [
    { q:"מה עיר הבירה של טורקיה?", a:"אנקרה", opts:["איסטנבול", "איזמיר", "בורסה"], diff:"בינוני" },
    { q:"מי כוכב נבחרת טורקיה?", a:"הכאן צ'לחנאוגלו", opts:["ארדא גולר", "פרדי קדיוגלו", "כנאן ילדיז"], diff:"קל" },
    { q:"טורקיה הגיעה לחצי גמר מונדיאל:", a:"2002", opts:["1998", "2006", "2010"], diff:"בינוני" },
    { q:"כמה תושבים יש בטורקיה (בערך)?", a:"85 מיליון", opts:["40 מיליון", "60 מיליון", "120 מיליון"], diff:"בינוני" },
    { q:"טורקיה נמצאת בין אסיה ל:", a:"אירופה", opts:["אפריקה", "המזרח התיכון", "הים הכספי"], diff:"קל" },
    { q:"מה המבנה המפורסם באיסטנבול?", a:"איה סופיה", opts:["הקולוסיאום", "אקרופוליס", "הספינקס"], diff:"קל" },
    { q:"מה המאכל המפורסם מטורקיה?", a:"קבב", opts:["פיצה", "פלאפל", "קוסקוס"], diff:"קל" },
    { q:"ארדא גולר שחקן של איזה קבוצה?", a:"ריאל מדריד", opts:["ברצלונה", "ביירן מינכן", "מנצ'סטר סיטי"], diff:"קל" },
  ],
  "Germany": [
    { q:"כמה פעמים זכתה גרמניה בגביע העולם?", a:"4 פעמים", opts:["3 פעמים", "5 פעמים", "2 פעמים"], diff:"קל" },
    { q:"מה עיר הבירה של גרמניה?", a:"ברלין", opts:["מינכן", "המבורג", "פרנקפורט"], diff:"קל" },
    { q:"מי כוכב נבחרת גרמניה?", a:"פלוריאן וירץ", opts:["ג'מאל מוסיאלה", "קאי הברץ", "תומס מולר"], diff:"בינוני" },
    { q:"כמה תושבים יש בגרמניה (בערך)?", a:"84 מיליון", opts:["50 מיליון", "100 מיליון", "150 מיליון"], diff:"בינוני" },
    { q:"מה מותג הרכב הגרמני המפורסם?", a:"BMW / Mercedes / Volkswagen", opts:["Toyota", "Fiat", "Renault"], diff:"קל" },
    { q:"גרמניה ניצחה 7-1 את מי ב-2014?", a:"ברזיל", opts:["ארגנטינה", "ספרד", "צרפת"], diff:"קל" },
    { q:"מה הבונדסליגה?", a:"ליגת הכדורגל של גרמניה", opts:["ליגת ההוקי", "ליגת הסל", "ליגת הטניס"], diff:"קל" },
    { q:"מה הנהר שזורם דרך ברלין?", a:"שפרה", opts:["ריין", "דנובה", "אלבה"], diff:"קשה" },
  ],
  "Curaçao": [
    { q:"לאן שייכת קורסאו?", a:"הולנד", opts:["בריטניה", "צרפת", "ספרד"], diff:"בינוני" },
    { q:"מה עיר הבירה של קורסאו?", a:"וילמסטד", opts:["קרלייה", "בנדה אבאו", "אנסה"], diff:"קשה" },
    { q:"מי כוכב נבחרת קורסאו?", a:"טאהית' ג'ונג", opts:["לאנדרו בקונה", "ג'ורגן לוקאדיה", "אליי רום"], diff:"בינוני" },
    { q:"קורסאו נמצאת בים:", a:"הקריבי", opts:["האטלנטי", "הפסיפי", "ההודי"], diff:"בינוני" },
    { q:"כמה תושבים יש בקורסאו (בערך)?", a:"150 אלף", opts:["50 אלף", "1 מיליון", "5 מיליון"], diff:"קשה" },
    { q:"מה השפה הייחודית של קורסאו?", a:"פאפיאמנטו", opts:["קריאולית", "פיג'ין", "ספניול"], diff:"קשה" },
    { q:"קורסאו ידועה בתעשיית:", a:"תיירות וזיקוק נפט", opts:["דיג בלבד", "חקלאות", "ייצוא בננות"], diff:"קשה" },
    { q:"באיזו קונפדרציה משחקת קורסאו?", a:"CONCACAF", opts:["CONMEBOL", "UEFA", "CAF"], diff:"בינוני" },
  ],
  "Côte d'Ivoire": [
    { q:"מה עיר הבירה של חוף השנהב?", a:"יאמוסוקרו", opts:["אביג'אן", "בואקה", "דלואה"], diff:"קשה" },
    { q:"מי כוכב נבחרת חוף השנהב?", a:"סימון אדינגרה", opts:["פרנק קסייה", "סבסטיאן הלר", "ז'אן-מייקל סרי"], diff:"בינוני" },
    { q:"דידייה דרוגבה שיחק לאיזה מועדון?", a:"צ'לסי", opts:["ארסנל", "מנצ'סטר יונייטד", "ליברפול"], diff:"קל" },
    { q:"כמה פעמים זכתה חוף השנהב בגביע אפריקה?", a:"פעמיים", opts:["פעם אחת", "3 פעמים", "אף פעם"], diff:"בינוני" },
    { q:"כמה תושבים יש בחוף השנהב (בערך)?", a:"27 מיליון", opts:["10 מיליון", "50 מיליון", "100 מיליון"], diff:"בינוני" },
    { q:"מה ייצוא חוף השנהב המפורסם?", a:"קקאו", opts:["קפה", "זהב", "יהלומים"], diff:"בינוני" },
    { q:"חוף השנהב הייתה מושבה של:", a:"צרפת", opts:["בריטניה", "פורטוגל", "ספרד"], diff:"בינוני" },
    { q:"מה השפה הרשמית של חוף השנהב?", a:"צרפתית", opts:["אנגלית", "ערבית", "פורטוגזית"], diff:"קל" },
  ],
  "Ecuador": [
    { q:"מה עיר הבירה של אקוודור?", a:"קיטו", opts:["גואיאקיל", "קואנקה", "מנטה"], diff:"בינוני" },
    { q:"מי כוכב נבחרת אקוודור?", a:"מויסס קאיסדו", opts:["אנר ולנסיה", "פרביס אסטופיניאן", "פיירו הינקפייה"], diff:"בינוני" },
    { q:"על שם מה קרויה אקוודור?", a:"קו המשווה (Ecuador=Equator)", opts:["נהר", "עיר", "הר"], diff:"בינוני" },
    { q:"כמה תושבים יש באקוודור (בערך)?", a:"18 מיליון", opts:["5 מיליון", "30 מיליון", "60 מיליון"], diff:"בינוני" },
    { q:"איזה ארכיפלג מפורסם שייך לאקוודור?", a:"איי גלאפגוס", opts:["איי קנרי", "איי אזורים", "איי מלדיב"], diff:"קל" },
    { q:"מה ההר הגעש הפעיל המפורסם באקוודור?", a:"קוטופאקסי", opts:["קילימנג'רו", "אטנה", "פוג'י"], diff:"קשה" },
    { q:"אקוודור נמצאת ביבשת:", a:"דרום אמריקה", opts:["צפון אמריקה", "אמריקה המרכזית", "הים הקריבי"], diff:"קל" },
    { q:"מה המטבע של אקוודור?", a:"דולר אמריקאי", opts:["סוקרה", "פסו", "בוליבר"], diff:"קשה" },
  ],
  "Netherlands": [
    { q:"מה עיר הבירה של הולנד?", a:"אמסטרדם", opts:["האג", "רוטרדם", "אוטרכט"], diff:"קל" },
    { q:"מי כוכב נבחרת הולנד?", a:"קודי גאקפו", opts:["וירג'יל ואן דייק", "פרנקי דה יונג", "חאווי סימונס"], diff:"בינוני" },
    { q:"כמה פעמים הגיעה הולנד לגמר מונדיאל?", a:"3 פעמים", opts:["פעמיים", "4 פעמים", "פעם אחת"], diff:"בינוני" },
    { q:"מה הפרח הסמלי של הולנד?", a:"צבעוני (טוליפ)", opts:["ורד", "חמנייה", "אורכידיאה"], diff:"קל" },
    { q:"כמה תושבים יש בהולנד (בערך)?", a:"17.9 מיליון", opts:["10 מיליון", "25 מיליון", "40 מיליון"], diff:"בינוני" },
    { q:"מה ה-Total Football המפורסם?", a:"סגנון כדורגל הולנדי שבו כל שחקן יכול לשחק בכל עמדה", opts:["שיטת הגנה", "שיטת לחץ", "שיטת נגד התקפות"], diff:"בינוני" },
    { q:"מה הכינוי של נבחרת הולנד?", a:"Oranje", opts:["Windmills", "Tulips", "Flying Dutchmen"], diff:"בינוני" },
    { q:"יוהאן קרויף שיחק לאיזה קבוצות?", a:"אייאקס וברצלונה", opts:["ריאל מדריד ומילאן", "ריאל מדריד וברצלונה", "אייאקס ומילאן"], diff:"בינוני" },
  ],
  "Japan": [
    { q:"מה עיר הבירה של יפן?", a:"טוקיו", opts:["אוסקה", "קיוטו", "יוקוהמה"], diff:"קל" },
    { q:"מי כוכב נבחרת יפן?", a:"קאורו מיטומה", opts:["דאיצ'י קמאדה", "טאקפוסה קובו", "איאסה אואדה"], diff:"בינוני" },
    { q:"יפן הפתיעה ב-2022 ב-WC כשניצחה:", a:"גרמניה וספרד", opts:["ברזיל וצרפת", "ארגנטינה וגרמניה", "בלגיה וגרמניה"], diff:"קל" },
    { q:"כמה תושבים יש ביפן (בערך)?", a:"125 מיליון", opts:["80 מיליון", "200 מיליון", "300 מיליון"], diff:"בינוני" },
    { q:"מה ההר המפורסם ביפן?", a:"הר פוג'י", opts:["הר אוליאמפוס", "הר ויזוב", "הר אנה"], diff:"קל" },
    { q:"מה הספורט המסורתי של יפן?", a:"סומו", opts:["ג'ודו", "קרטה", "קנדו"], diff:"קל" },
    { q:"מה J-League?", a:"כדורגל יפנית", opts:["בייסבול", "היאבקות", "רוגבי"], diff:"קל" },
    { q:"מה סגנון הסוגה המפורסם מיפן?", a:"סושי", opts:["טמפורה", "ראמן", "טאקויאקי"], diff:"קל" },
  ],
  "Sweden": [
    { q:"מה עיר הבירה של שוודיה?", a:"סטוקהולם", opts:["גטבורג", "מלמו", "אופסלה"], diff:"קל" },
    { q:"מי כוכב נבחרת שוודיה?", a:"ויקטור גיוקרס", opts:["אלכסנדר איסאק", "דיאן קולוסבסקי", "אמיל פורסברג"], diff:"בינוני" },
    { q:"מי הכוכב הגדול של שוודיה בכל הזמנים?", a:"זלטן איברהימוביץ'", opts:["הנריק לארסון", "פרדריק לייונברג", "אנדריאס גרנקוויסט"], diff:"קל" },
    { q:"כמה תושבים יש בשוודיה (בערך)?", a:"10.5 מיליון", opts:["5 מיליון", "20 מיליון", "30 מיליון"], diff:"בינוני" },
    { q:"מה חברת הרהיטים המפורסמת משוודיה?", a:"IKEA", opts:["H&M", "ZARA", "Primark"], diff:"קל" },
    { q:"מה הלהקה המפורסמת משוודיה?", a:"ABBA", opts:["The Beatles", "Rolling Stones", "Coldplay"], diff:"קל" },
    { q:"שוודיה ממוקמת בחצי האי:", a:"סקנדינביה", opts:["האיברי", "הבלקני", "איטלקי"], diff:"קל" },
    { q:"מה פרס נובל קשור לשוודיה?", a:"מוענק בשטוקהולם", opts:["הוחל בשוודיה", "על שם שוודי", "שניים נכונים"], diff:"בינוני" },
  ],
  "Tunisia": [
    { q:"מה עיר הבירה של תוניסיה?", a:"תוניס", opts:["ספאקס", "סוסה", "קאירואן"], diff:"קל" },
    { q:"מי כוכב נבחרת תוניסיה?", a:"חניבל מג'בי", opts:["ואהבי ח'זרי", "אליס שחרי", "אייסה לידוני"], diff:"בינוני" },
    { q:"תוניסיה נמצאת בצפון:", a:"אפריקה", opts:["המזרח התיכון", "אירופה", "אסיה"], diff:"קל" },
    { q:"כמה תושבים יש בתוניסיה (בערך)?", a:"12 מיליון", opts:["5 מיליון", "25 מיליון", "50 מיליון"], diff:"בינוני" },
    { q:"תוניסיה הייתה הנבחרת הערבית הראשונה שניצחה ב-WC נגד:", a:"מקסיקו (1978)", opts:["גרמניה", "ספרד", "ברזיל"], diff:"קשה" },
    { q:"מה הים שגובל בתוניסיה מהצפון?", a:"הים התיכון", opts:["הים האדום", "האוקיינוס האטלנטי", "המפרץ הפרסי"], diff:"קל" },
    { q:"תוניסיה הייתה מושבה של:", a:"צרפת", opts:["בריטניה", "איטליה", "ספרד"], diff:"בינוני" },
    { q:"מה המאכל התוניסאי המפורסם?", a:"חריסה", opts:["קוסקוס", "פלאפל", "טאג'ין"], diff:"בינוני" },
  ],
  "France": [
    { q:"כמה פעמים זכתה צרפת בגביע העולם?", a:"פעמיים", opts:["פעם אחת", "3 פעמים", "4 פעמים"], diff:"קל" },
    { q:"מה עיר הבירה של צרפת?", a:"פריז", opts:["ליון", "מרסיי", "ניס"], diff:"קל" },
    { q:"מי כוכב נבחרת צרפת?", a:"קיליאן מבאפה", opts:["אנטואן גריזמן", "מרקוס תוראם", "ואוסמן דמבלה"], diff:"קל" },
    { q:"כמה תושבים יש בצרפת (בערך)?", a:"68 מיליון", opts:["40 מיליון", "50 מיליון", "100 מיליון"], diff:"בינוני" },
    { q:"מה המגדל המפורסם בפריז?", a:"מגדל אייפל", opts:["מגדל בג 'בן", "מגדל פיזה", "מגדל לונדון"], diff:"קל" },
    { q:"מה הכינוי של נבחרת צרפת?", a:"Les Bleus", opts:["Les Rouges", "Les Tricolores", "Les Coqs"], diff:"בינוני" },
    { q:"צרפת זכתה לראשונה ב-WC בשנת:", a:"1998", opts:["1986", "2006", "2014"], diff:"קל" },
    { q:"זינדין זידאן ידוע גם בשל:", a:"נגיחה בגמר 2006", opts:["שתי זכיות ב-WC", "שלושה כדורי זהב", "הבקעת הטריק בגמר"], diff:"בינוני" },
  ],
  "Senegal": [
    { q:"מה עיר הבירה של סנגל?", a:"דקאר", opts:["תיס", "זיגינשור", "סאן לואי"], diff:"בינוני" },
    { q:"מי כוכב נבחרת סנגל?", a:"סאדיו מאנה", opts:["ניקולס ג'קסון", "איסמאילה סאר", "פאפה מאטאר סאר"], diff:"קל" },
    { q:"סנגל זכתה בגביע אפריקה:", a:"2021", opts:["2015", "2012", "2019"], diff:"בינוני" },
    { q:"כמה תושבים יש בסנגל (בערך)?", a:"17 מיליון", opts:["5 מיליון", "30 מיליון", "60 מיליון"], diff:"בינוני" },
    { q:"סנגל הגיעה לרבע גמר מונדיאל בשנת:", a:"2002", opts:["2018", "2022", "2010"], diff:"בינוני" },
    { q:"מה הים שגובל בסנגל מהמערב?", a:"האוקיינוס האטלנטי", opts:["הים התיכון", "הים האדום", "המפרץ הגיני"], diff:"בינוני" },
    { q:"סנגל הייתה מושבה של:", a:"צרפת", opts:["בריטניה", "פורטוגל", "בלגיה"], diff:"בינוני" },
    { q:"מה השפה הרשמית של סנגל?", a:"צרפתית", opts:["ערבית", "וולוף", "אנגלית"], diff:"בינוני" },
  ],
  "Iraq": [
    { q:"מה עיר הבירה של עיראק?", a:"בגדאד", opts:["בצרה", "מוסול", "ארביל"], diff:"קל" },
    { q:"מי כוכב נבחרת עיראק?", a:"עלי אל-חמאדי", opts:["איימן חוסיין", "מוחנד עלי", "בשאר רסאן"], diff:"קשה" },
    { q:"עיראק זכתה בגביע אסיה:", a:"2007", opts:["2011", "2015", "2019"], diff:"קשה" },
    { q:"כמה תושבים יש בעיראק (בערך)?", a:"42 מיליון", opts:["15 מיליון", "70 מיליון", "100 מיליון"], diff:"בינוני" },
    { q:"מה הנהרות הגדולים שזורמים בעיראק?", a:"חידקל ופרת", opts:["נילוס ואמזונס", "ריין ודנובה", "ינגצה ויאנגטצי"], diff:"בינוני" },
    { q:"מה ציוויליזציה עתיקה שהייתה בעיראק?", a:"מסופוטמיה", opts:["מצרים העתיקה", "יוון העתיקה", "רומא העתיקה"], diff:"קל" },
    { q:"באיזו קונפדרציה משחקת עיראק?", a:"AFC", opts:["CAF", "CONCACAF", "UEFA"], diff:"בינוני" },
    { q:"מה הדת הדומיננטית בעיראק?", a:"אסלאם", opts:["נצרות", "יהדות", "בודהיזם"], diff:"קל" },
  ],
  "Norway": [
    { q:"מה עיר הבירה של נורווגיה?", a:"אוסלו", opts:["ברגן", "טרונדהיים", "סטוונגר"], diff:"קל" },
    { q:"מי כוכב נבחרת נורווגיה?", a:"ארלינג הולאנד", opts:["מרטין אודגארד", "אלכסנדר זורלות", "סנדר ברגה"], diff:"קל" },
    { q:"כמה תושבים יש בנורווגיה (בערך)?", a:"5.5 מיליון", opts:["2 מיליון", "10 מיליון", "20 מיליון"], diff:"בינוני" },
    { q:"נורווגיה ידועה בתופעת:", a:"אורות הצפון", opts:["השמש השוקעת", "ירח הדם", "שמש חצות"], diff:"קל" },
    { q:"מה המשאב הטבעי העיקרי של נורווגיה?", a:"נפט וגז מהים הצפוני", opts:["פחם", "זהב", "עצים"], diff:"בינוני" },
    { q:"נורווגיה ידועה גם בספורט:", a:"סקי ואולימפיאדת חורף", opts:["שחייה", "היאבקות", "קנועינג"], diff:"קל" },
    { q:"פרס נובל לשלום מוענק בנורווגיה, בעיר:", a:"אוסלו", opts:["ברגן", "סטוקהולם", "הלסינקי"], diff:"בינוני" },
    { q:"מה הסיפור המיתולוגי הקשור לנורווגיה?", a:"ויקינגים ואלים נורדיים", opts:["אולימפוס", "ואלהאלה", "אסגארד"], diff:"קל" },
  ],
  "Argentina": [
    { q:"כמה פעמים זכתה ארגנטינה בגביע העולם?", a:"3 פעמים", opts:["פעמיים", "4 פעמים", "5 פעמים"], diff:"קל" },
    { q:"מה עיר הבירה של ארגנטינה?", a:"בואנוס איירס", opts:["קורדובה", "רוסריו", "מנדוזה"], diff:"קל" },
    { q:"מי כוכב נבחרת ארגנטינה?", a:"ליונל מסי", opts:["ליאוטארו מרטינז", "חוליאן אלברז", "רוחריגו דה פול"], diff:"קל" },
    { q:"כמה תושבים יש בארגנטינה (בערך)?", a:"46 מיליון", opts:["20 מיליון", "80 מיליון", "100 מיליון"], diff:"בינוני" },
    { q:"ארגנטינה זכתה לאחרונה ב-WC בשנת:", a:"2022", opts:["2014", "1986", "2006"], diff:"קל" },
    { q:"מה הריקוד המפורסם מארגנטינה?", a:"טנגו", opts:["סלסה", "בולרו", "למבאדה"], diff:"קל" },
    { q:"מה הנהר שיוצר גבול טבעי בארגנטינה?", a:"ריו דה לה פלטה", opts:["הפרנה", "האמזונס", "האורינוקו"], diff:"בינוני" },
    { q:"מרדונה נחשב לאחד הטובים בכל הזמנים, נולד ב:", a:"1960", opts:["1950", "1970", "1965"], diff:"קשה" },
  ],
  "Algeria": [
    { q:"מה עיר הבירה של אלג'יריה?", a:"אלג'יר", opts:["אורן", "קונסטנטין", "אנאבה"], diff:"בינוני" },
    { q:"מי כוכב נבחרת אלג'יריה?", a:"ריאד מהרז", opts:["אסלאם סלימאני", "ישמאל בנאסר", "נביל בנטלב"], diff:"קל" },
    { q:"אלג'יריה זכתה בגביע אפריקה:", a:"2019", opts:["2015", "2012", "2022"], diff:"בינוני" },
    { q:"כמה תושבים יש באלג'יריה (בערך)?", a:"46 מיליון", opts:["20 מיליון", "70 מיליון", "100 מיליון"], diff:"בינוני" },
    { q:"אלג'יריה היא המדינה הגדולה ביבשת:", a:"אפריקה", opts:["אסיה", "דרום אמריקה", "אירופה"], diff:"בינוני" },
    { q:"מה המדבר הגדול שעובר דרך אלג'יריה?", a:"הסהרה", opts:["גובי", "נגב", "קלהארי"], diff:"קל" },
    { q:"אלג'יריה הייתה מושבה של:", a:"צרפת", opts:["ספרד", "בריטניה", "איטליה"], diff:"בינוני" },
    { q:"מה השפה הרשמית של אלג'יריה?", a:"ערבית", opts:["צרפתית", "ברברית", "עמהרית"], diff:"קל" },
  ],
  "Austria": [
    { q:"מה עיר הבירה של אוסטריה?", a:"וינה", opts:["גראץ", "זלצבורג", "לינץ"], diff:"קל" },
    { q:"מי כוכב נבחרת אוסטריה?", a:"מרסל זבניצר", opts:["קונרד לאיימר", "כריסטוף באומגרטנר", "דוד אלאבה"], diff:"בינוני" },
    { q:"אוסטריה ידועה במוזיקה של:", a:"מוצרט ובטהובן", opts:["באך", "שופן", "ברהמס"], diff:"בינוני" },
    { q:"כמה תושבים יש באוסטריה (בערך)?", a:"9 מיליון", opts:["3 מיליון", "20 מיליון", "40 מיליון"], diff:"בינוני" },
    { q:"אוסטריה ידועה גם בספורט:", a:"סקי אלפיני", opts:["גלישה", "שחייה", "קיקבוקסינג"], diff:"קל" },
    { q:"מה הנהר שעובר בוינה?", a:"הדנובה", opts:["הריין", "האלבה", "הוויסלה"], diff:"בינוני" },
    { q:"אוסטריה גובלת עם כמה מדינות?", a:"8", opts:["4", "5", "6"], diff:"קשה" },
    { q:"מה הכינוי של נבחרת אוסטריה?", a:"הנשרים", opts:["הדובים", "הנמרים", "האריות"], diff:"קשה" },
  ],
  "Jordan": [
    { q:"מה עיר הבירה של ירדן?", a:"עמאן", opts:["זרקא", "אירביד", "אל-עקבה"], diff:"קל" },
    { q:"מי כוכב נבחרת ירדן?", a:"מוסא אל-תמארי", opts:["יאסר אל-נעימאת", "עמד עטוואן", "באהא פייסל"], diff:"קשה" },
    { q:"ירדן הגיעה לגמר גביע אסיה:", a:"2023", opts:["2019", "2015", "2011"], diff:"בינוני" },
    { q:"כמה תושבים יש בירדן (בערך)?", a:"10 מיליון", opts:["3 מיליון", "20 מיליון", "50 מיליון"], diff:"בינוני" },
    { q:"מה האתר הארכאולוגי המפורסם בירדן?", a:"פטרה", opts:["הפירמידות", "קוה-אי-נור", "ג'רש"], diff:"קל" },
    { q:"מה הים הנמוך בעולם שגובל בירדן?", a:"ים המלח", opts:["ים קספי", "ים מת", "ים אדום"], diff:"קל" },
    { q:"ירדן גובלת עם ישראל?", a:"כן", opts:["לא", "רק בגדה המערבית", "רק בעמק הירדן"], diff:"קל" },
    { q:"מה הנהר שמסמן גבול ירדן-ישראל?", a:"הירדן", opts:["הכינרת", "הארנון", "היבוק"], diff:"בינוני" },
  ],
  "Portugal": [
    { q:"כמה פעמים זכתה פורטוגל ביורו?", a:"פעמיים", opts:["פעם אחת", "3 פעמים", "אף פעם"], diff:"בינוני" },
    { q:"מה עיר הבירה של פורטוגל?", a:"ליסבון", opts:["פורטו", "ברגה", "קואימברה"], diff:"קל" },
    { q:"מי כוכב נבחרת פורטוגל?", a:"ברונו פרננדס", opts:["כריסטיאנו רונאלדו", "בר", "רובן דיאש"], diff:"בינוני" },
    { q:"כמה תושבים יש בפורטוגל (בערך)?", a:"10 מיליון", opts:["5 מיליון", "20 מיליון", "40 מיליון"], diff:"בינוני" },
    { q:"מה הסגנון המוזיקלי המסורתי של פורטוגל?", a:"פאדו", opts:["פלמנקו", "סמבה", "בוסה נובה"], diff:"בינוני" },
    { q:"פורטוגל נמצאת בחצי האי:", a:"האיברי", opts:["האפנינים", "הבלקני", "סקנדינביה"], diff:"בינוני" },
    { q:"מה האוקיינוס שגובל בפורטוגל מהמערב?", a:"האטלנטי", opts:["הפסיפי", "ההודי", "הארקטי"], diff:"קל" },
    { q:"רונאלדו שיחק כמה שנים בריאל מדריד?", a:"9 שנים (2009-2018)", opts:["5 שנים", "7 שנים", "12 שנים"], diff:"בינוני" },
  ],
  "DR Congo": [
    { q:"מה עיר הבירה של קונגו?", a:"קינשאסה", opts:["לובומבאשי", "גומה", "קנגה"], diff:"בינוני" },
    { q:"מי כוכב נבחרת קונגו?", a:"יואן ויסה", opts:["תיאו בונגונדה", "מישי בצ'ואי", "ג'ואל קיאסומבואה"], diff:"בינוני" },
    { q:"קונגו ידועה בנהר:", a:"הקונגו", opts:["הנילוס", "הניז'ר", "הזמבזי"], diff:"קל" },
    { q:"כמה תושבים יש בקונגו (בערך)?", a:"100 מיליון", opts:["30 מיליון", "60 מיליון", "200 מיליון"], diff:"בינוני" },
    { q:"קונגו הייתה מושבה של:", a:"בלגיה", opts:["צרפת", "בריטניה", "פורטוגל"], diff:"בינוני" },
    { q:"מה יערות הגשם שנמצאים בקונגו?", a:"יערות גשם טרופיים — שני בגודלם בעולם", opts:["יערות אמזונס", "יערות בורנאו", "יערות פפואה"], diff:"קשה" },
    { q:"מה הכינוי של נבחרת קונגו?", a:"נמרי האטום", opts:["אריות אפריקה", "הנשרים", "לאופארד"], diff:"קשה" },
    { q:"קונגו נמצאת ביבשת:", a:"אפריקה", opts:["אסיה", "דרום אמריקה", "אוקיאניה"], diff:"קל" },
  ],
  "Uzbekistan": [
    { q:"מה עיר הבירה של אוזבקיסטן?", a:"טשקנט", opts:["סמרקנד", "בוכרה", "נמנגן"], diff:"בינוני" },
    { q:"מי כוכב נבחרת אוזבקיסטן?", a:"אלדור שומורודוב", opts:["עבוסבק פייזולייב", "אוטאבק שוקורוב", "ג'משיד איסקנדרוב"], diff:"קשה" },
    { q:"אוזבקיסטן שייכת לאזור:", a:"אסיה המרכזית", opts:["המזרח הרחוק", "דרום מזרח אסיה", "הקווקז"], diff:"בינוני" },
    { q:"כמה תושבים יש באוזבקיסטן (בערך)?", a:"36 מיליון", opts:["10 מיליון", "60 מיליון", "100 מיליון"], diff:"בינוני" },
    { q:"מה הדרך המסחרית המפורסמת שעברה דרך אוזבקיסטן?", a:"דרך המשי", opts:["דרך התבלינים", "דרך הזהב", "דרך הרוח"], diff:"קל" },
    { q:"מה העיר ההיסטורית המפורסמת באוזבקיסטן?", a:"סמרקנד", opts:["בוכרה", "טשקנט", "חיווא"], diff:"בינוני" },
    { q:"אוזבקיסטן הייתה חלק מ:", a:"ברית המועצות", opts:["האימפריה הבריטית", "האימפריה העות'מאנית", "האימפריה הפרסית"], diff:"בינוני" },
    { q:"באיזו קונפדרציה משחקת אוזבקיסטן?", a:"AFC", opts:["UEFA", "CAF", "CONCACAF"], diff:"בינוני" },
  ],
  "Colombia": [
    { q:"מה עיר הבירה של קולומביה?", a:"בוגוטה", opts:["מדיין", "קאלי", "קרטחנה"], diff:"קל" },
    { q:"מי כוכב נבחרת קולומביה?", a:"לואיס דיאז", opts:["חוואן קואדראדו", "דאווינסון סנצ'ז", "ריצ'רד ריוס"], diff:"קל" },
    { q:"קולומביה הגיעה לרבע גמר מונדיאל:", a:"2014", opts:["2018", "2006", "1994"], diff:"בינוני" },
    { q:"כמה תושבים יש בקולומביה (בערך)?", a:"52 מיליון", opts:["20 מיליון", "80 מיליון", "120 מיליון"], diff:"בינוני" },
    { q:"קולומביה ידועה בייצוא:", a:"קפה", opts:["קקאו", "טבק", "סוכר"], diff:"קל" },
    { q:"קולומביה נמצאת בצפון:", a:"דרום אמריקה", opts:["אמריקה המרכזית", "הים הקריבי", "צפון אמריקה"], diff:"קל" },
    { q:"קרטחנה בקולומביה ידועה בשל:", a:"העיר ההיסטורית המוקפת חומה", opts:["חופי הגלישה", "הג'ונגל", "ענף הספורט"], diff:"בינוני" },
    { q:"ג'יימס רודריגס זכה בפרס:", a:"מלך השערים ב-WC 2014", opts:["הכדורגלן הטוב ב-WC", "השומר הטוב", "הכוכב הצעיר"], diff:"בינוני" },
  ],
  "England": [
    { q:"כמה פעמים זכתה אנגליה בגביע העולם?", a:"פעם אחת", opts:["פעמיים", "3 פעמים", "אף פעם"], diff:"קל" },
    { q:"מה עיר הבירה של אנגליה?", a:"לונדון", opts:["מנצ'סטר", "ליברפול", "בירמינגהם"], diff:"קל" },
    { q:"מי כוכב נבחרת אנגליה?", a:"ג'וד בלינגהם", opts:["הארי קיין", "בוקאיו סאקה", "פיל פודן"], diff:"קל" },
    { q:"כמה תושבים יש באנגליה (בערך)?", a:"57 מיליון", opts:["30 מיליון", "80 מיליון", "100 מיליון"], diff:"בינוני" },
    { q:"אנגליה זכתה ב-WC בשנת:", a:"1966", opts:["1970", "1990", "1982"], diff:"קל" },
    { q:"מה הכינוי של נבחרת אנגליה?", a:"Three Lions", opts:["Red Devils", "The Eagles", "St. George's Cross"], diff:"קל" },
    { q:"הפרמייר ליג הוקמה בשנת:", a:"1992", opts:["1985", "1998", "1975"], diff:"בינוני" },
    { q:"מה האצטדיון הלאומי של אנגליה?", a:"וומבלי", opts:["אולד טראפורד", "סטמפורד ברידג", "אנפילד"], diff:"קל" },
  ],
  "Croatia": [
    { q:"מה עיר הבירה של קרואטיה?", a:"זגרב", opts:["ספליט", "דוברובניק", "ריאקה"], diff:"קל" },
    { q:"מי כוכב נבחרת קרואטיה?", a:"לוקה מודריץ'", opts:["מרסלו ברוזוביץ'", "מאטיאו קוביציץ'", "יושקו גווארדיול"], diff:"קל" },
    { q:"קרואטיה הגיעה לגמר מונדיאל:", a:"2018", opts:["1998", "2006", "2022"], diff:"בינוני" },
    { q:"כמה תושבים יש בקרואטיה (בערך)?", a:"4 מיליון", opts:["1 מיליון", "10 מיליון", "20 מיליון"], diff:"בינוני" },
    { q:"קרואטיה ידועה בחוף:", a:"הים האדריאטי", opts:["הים התיכון", "הים השחור", "הים האגאי"], diff:"בינוני" },
    { q:"דוברובניק ידועה כ:", a:"פנינת הים האדריאטי", opts:["עיר ההר", "עיר הנמל", "עיר הזהב"], diff:"בינוני" },
    { q:"קרואטיה הייתה חלק מ:", a:"יוגוסלביה", opts:["ברית המועצות", "האימפריה העות'מאנית", "ממלכת הונגריה"], diff:"בינוני" },
    { q:"לוקה מודריץ' זכה בכדור הזהב:", a:"2018", opts:["2016", "2020", "2022"], diff:"בינוני" },
  ],
  "Ghana": [
    { q:"מה עיר הבירה של גאנה?", a:"אקרה", opts:["קומאסי", "טאמאלה", "סקונדי"], diff:"בינוני" },
    { q:"מי כוכב נבחרת גאנה?", a:"מוחמד קודוס", opts:["תומאס פרטי", "אינאקי וויליאמס", "אנדרה אאיו"], diff:"קל" },
    { q:"גאנה הגיעה לרבע גמר WC:", a:"2010", opts:["2006", "2014", "2022"], diff:"בינוני" },
    { q:"כמה תושבים יש בגאנה (בערך)?", a:"33 מיליון", opts:["10 מיליון", "60 מיליון", "100 מיליון"], diff:"בינוני" },
    { q:"גאנה הייתה המדינה הראשונה בדרום הסהרה שהשיגה עצמאות:", a:"1957", opts:["1945", "1965", "1970"], diff:"קשה" },
    { q:"מה מייצאת גאנה בכמויות גדולות?", a:"קקאו וזהב", opts:["נפט בלבד", "יהלומים", "עץ"], diff:"בינוני" },
    { q:"הכינוי של נבחרת גאנה:", a:"הכוכבים השחורים", opts:["הנמרים", "הנשרים", "האריות"], diff:"בינוני" },
    { q:"גאנה הייתה מושבה של:", a:"בריטניה", opts:["צרפת", "פורטוגל", "בלגיה"], diff:"בינוני" },
  ],
  "Panama": [
    { q:"מה עיר הבירה של פנמה?", a:"עיר פנמה", opts:["קולון", "דוויד", "צ'יטרה"], diff:"קל" },
    { q:"מי כוכב נבחרת פנמה?", a:"אדלברטו קרסקילה", opts:["ססיליו ווטרמן", "מייקל מוריו", "רולנדו בלקברן"], diff:"קשה" },
    { q:"מה התעלה המפורסמת שנמצאת בפנמה?", a:"תעלת פנמה", opts:["תעלת סואץ", "תעלת קיל", "תעלת מנצ'סטר"], diff:"קל" },
    { q:"כמה תושבים יש בפנמה (בערך)?", a:"4.3 מיליון", opts:["1 מיליון", "10 מיליון", "20 מיליון"], diff:"קשה" },
    { q:"פנמה מחברת בין:", a:"צפון אמריקה ודרום אמריקה", opts:["קריבי לאוקיינוס", "אמריקה לאירופה", "אטלנטי לפסיפי"], diff:"קל" },
    { q:"פנמה השתתפה לראשונה ב-WC בשנת:", a:"2018", opts:["2006", "2010", "2014"], diff:"בינוני" },
    { q:"מה הסמל הלאומי של פנמה?", a:"הרפיה (עיט ענק)", opts:["נמר", "קנגורו", "פינגווין"], diff:"קשה" },
    { q:"פנמה שייכת לקונפדרציה:", a:"CONCACAF", opts:["CONMEBOL", "UEFA", "AFC"], diff:"בינוני" },
  ],
};
  ...
  function handleAnswer(idx) {
    if (answered) return;
    setAnswered(true); setChosenIdx(idx);
    const q = questions[current];
  ...
  function useLifeline(type) {
    ...
    if (type==="skip") {
      setAnswered(true);
      setTimeout(()=>{setCurrent(p=>p+1);setAnswered(false);setChosenIdx(null);setHiddenOpts([]);setAudiencePcts(null);setTimeLeft(quizType==="flags"?10:20);},100);
    } else if (type==="fifty") {
      const wrong=shuffledOpts.map((_,i)=>i).filter(i=>shuffledOpts[i]!==q.a);
      setHiddenOpts(shuffleArr(wrong).slice(0,2));
    } else if (type==="audience") {
      const ci=shuffledOpts.findIndex(o=>o===q.a);
      const pcts=shuffledOpts.map((_,i)=>i===ci?52+Math.floor(Math.random()*25):5+Math.floor(Math.random()*12));
  ...
  // Guard: if no question found, go back home
  if ((phase === "flags" || phase === "general") && !q) {
    return (
      <div style={{position:"fixed",inset:0,zIndex:9200,background:"#050912",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16}}>
        <div style={{fontSize:32}}>⏳</div>
        <div style={{color:"#94a3b8",fontSize:14}}>טוען שאלות...</div>
        <button onClick={()=>setPhase("home")} style={{marginTop:8,padding:"10px 20px",borderRadius:10,background:"rgba(30,41,59,0.8)",border:"1px solid rgba(71,85,105,0.4)",color:"#94a3b8",cursor:"pointer",fontFamily:"inherit"}}>חזור</button>
      </div>
    );
  }
      ...
      <div style={{padding:"20px 16px",display:"flex",flexDirection:"column",gap:14}}>
        ...
        {(() => {
          ...
          Object.entries(SCHEDULE).forEach(([key, val]) => {
            const d = new Date(val.kickoff).toISOString().slice(0,10);
            if (d === todayStr) {
              const [h, a] = key.split("|");
              if (!todayTeams.includes(h)) todayTeams.push(h);
              if (!todayTeams.includes(a)) todayTeams.push(a);
            }
          });
          const dailyKey = `dailyQuiz_${todayStr}`;
          const doneToday = !!localStorage.getItem(dailyKey);
          const startDaily = () => {
            let qs = [];
          ...
          return (
            <div style={{background:"rgba(217,119,6,0.08)",border:"1.5px solid rgba(251,191,36,0.4)",borderRadius:18,padding:"18px 16px"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                <div style={{fontSize:32}}>📅</div>
                <div style={{background:"rgba(251,191,36,0.15)",border:"1px solid rgba(251,191,36,0.4)",borderRadius:20,padding:"3px 12px",fontSize:11,fontWeight:800,color:"#fbbf24"}}>🆓 חינם יומי</div>
              </div>
              ...
              {todayTeams.length > 0 && (
                <div style={{display:"flex",flexWrap:"wrap",gap:4,marginBottom:10}}>
                  {todayTeams.map(t => (
                    <span key={t} style={{background:"rgba(251,191,36,0.1)",border:"1px solid rgba(251,191,36,0.3)",borderRadius:6,padding:"2px 7px",fontSize:10,color:"#fbbf24",fontWeight:700}}>{t}</span>
                  ))}
                </div>
              )}
              {doneToday ? (
                <div style={{textAlign:"center",padding:"10px",fontSize:12,color:"#fbbf24",fontWeight:700}}>
                  ✅ שיחקת היום! חזור מחר 👋
                </div>
              ) : (
                <button onClick={startDaily} style={{width:"100%",padding:"14px",borderRadius:13,background:"linear-gradient(135deg,#b45309,#d97706)",border:"none",color:"#fff",fontSize:15,fontWeight:900,fontFamily:"inherit",cursor:"pointer",boxShadow:"0 6px 20px rgba(217,119,6,0.4)"}}>
                  📅 התחל חידון יומי
                </button>
              )}
        ...
        <div style={{background:"rgba(34,197,94,0.08)",border:"1.5px solid rgba(34,197,94,0.35)",borderRadius:18,padding:"18px 16px"}}>
          ...
          <div style={{background:"rgba(34,197,94,0.08)",borderRadius:12,padding:"10px 12px",marginBottom:12}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:5,fontSize:12,color:"#cbd5e1"}}>
              <div style={{display:"flex",justifyContent:"space-between"}}><span>10 נכון</span><span style={{color:"#fbbf24",fontWeight:800}}>💰 500</span></div>
              ...
              <div style={{height:1,background:"rgba(34,197,94,0.15)"}}/>
              <div style={{display:"flex",justifyContent:"space-between"}}><span>מעל 20</span><span style={{color:"#c084fc",fontWeight:800}}>🏆 שיא!</span></div>
            </div>
          </div>
          <button onClick={()=>{setQuizType("flags");const q=shuffleArr(QUIZ_FLAGS);setQuestions(q);setCurrent(0);setCorrect(0);setLives(3);setAnswered(false);setChosenIdx(null);setHiddenOpts([]);setAudiencePcts(null);setLifelines({skip:false,fifty:false,audience:false});setTimeLeft(10);setPhase("flags");}} style={{width:"100%",padding:"14px",borderRadius:13,background:"linear-gradient(135deg,#16a34a,#22c55e)",border:"none",color:"#fff",fontSize:16,fontWeight:900,fontFamily:"inherit",cursor:"pointer",boxShadow:"0 6px 20px rgba(34,197,94,0.4)"}}>
            🌍 התחל דגלים
          </button>
        </div>
        ...
        <div style={{background:"rgba(99,102,241,0.08)",border:"1.5px solid rgba(99,102,241,0.35)",borderRadius:18,padding:"18px 16px"}}>
          ...
          <div style={{fontSize:12,color:"#94a3b8",marginBottom:12}}>20 שניות • 3 גלגלי הצלה • קושי עולה</div>
          <div style={{background:tokens>0?"rgba(251,191,36,0.1)":"rgba(99,102,241,0.08)",border:`1px solid ${tokens>0?"rgba(251,191,36,0.4)":"rgba(99,102,241,0.3)"}`,borderRadius:12,padding:"9px 12px",marginBottom:12,display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:20}}>{tokens>0?"🎫":"🪙"}</span>
            <div>
              <div style={{fontSize:12,fontWeight:900,color:tokens>0?"#fbbf24":"#94a3b8"}}>{tokens>0?`${tokens} טוקן — כניסה חינם`:"1,000 מטבעות או טוקן מדגלים"}</div>
            </div>
          </div>
          ...
          <button onClick={()=>{
            ...
            setLifelines({skip:true,fifty:true,audience:true});
            setTimeLeft(20);setPhase("general");
          }} style={{width:"100%",padding:"14px",borderRadius:13,background:"rgba(30,41,59,0.8)",color:"#cbd5e1",border:"1px solid rgba(99,102,241,0.4)",fontSize:14,fontWeight:900,fontFamily:"inherit",cursor:"pointer"}}>
              🪙 התחל — 1,000 מטבעות
            </button>
          )}
        </div>
    ...
    <div style={{position:"fixed",inset:0,zIndex:9200,background:"linear-gradient(180deg,#050912,#0f172a)",display:"flex",flexDirection:"column",padding:"10px 16px 12px",gap:8}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,background:"rgba(15,23,42,0.6)",border:"1px solid rgba(71,85,105,0.3)",borderRadius:14,padding:"8px 12px",flexShrink:0}}>
        <div style={{position:"relative",width:42,height:42,flexShrink:0}}>
          <svg viewBox="0 0 42 42" width="42" height="42" style={{transform:"rotate(-90deg)"}}>
            ...
            <circle fill="none" stroke={timerColor} strokeWidth="4" strokeLinecap="round" cx="21" cy="21" r="18"
          ...
          <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",fontSize:14,fontWeight:900,color:timerColor}}>{timeLeft}</div>
        </div>
        <div style={{flex:1}}>
          ...
          <div style={{height:4,background:"rgba(71,85,105,0.3)",borderRadius:4,overflow:"hidden"}}>
            <div style={{height:"100%",width:(pct*100)+"%",background:timerColor,borderRadius:4,transition:"width 1s linear"}}/>
          </div>
        </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:7,flex:1}}>
        {opts.map((opt,i)=>{
          ...
          const isCorrectOpt=opt===q?.a, chosen=chosenIdx===i;
          let bg="rgba(30,41,59,0.65)",border="rgba(71,85,105,0.35)",tc="#e2e8f0";
          if (answered) {
            if (isCorrectOpt) {bg="rgba(34,197,94,0.18)";border="#22c55e";tc="#86efac";}
            else if (chosen) {bg="rgba(239,68,68,0.12)";border="#ef4444";tc="#fca5a5";}
            else {bg="rgba(30,41,59,0.3)";tc="#64748b";}
          }
          return (
            <button key={i} disabled={answered} onClick={()=>handleAnswer(i)} style={{display:"flex",alignItems:"center",gap:10,padding:"0 12px",borderRadius:13,border:`2px solid ${border}`,background:bg,color:tc,fontSize:13,fontWeight:700,fontFamily:"inherit",cursor:answered?"default":"pointer",flex:1,transition:"all 0.15s"}}>
      ...
      {!isFlags && (
        <div style={{display:"flex",gap:8,flexShrink:0}}>
          {[{key:"skip",icon:"⏭️",label:"דלג"},{key:"fifty",icon:"🎯",label:"50:50"},{key:"audience",icon:"👥",label:"קהל"}].map(ll=>(
...
function LuckyWheelModal({ onClose, onWin, onUpdateBestStreak, personalBest, leagueRecord, leaderboard = [], freePlaysLeft, coinBalance, onUseFree, onPayPlay }) {
  ...
  const renderMiniCard = (card) => {
    if (!card) return null;
    if (card.rarity === "B") return <PlayerCard card={card} size="M" animated={false} />;
...
function CollectionModal({ collection, onClose }) {
  const t = useT();
  const [section, setSection] = useState("players"); // "players" | "legends" | "ballon"
  const [filter, setFilter] = useState("all");
  const [previewCard, setPreviewCard] = useState(null);

  const BALLON_CARDS = CARDS_BY_RARITY.B;

  const filteredCards = useMemo(() => {
    const rarityOrder = { F: 0, X: 1, G: 2, L: 3, E: 4, R: 5, U: 6, C: 7, T: 8, B: -1 };
    const sorter = (a, b) => {
      const r = (rarityOrder[a.rarity] ?? 99) - (rarityOrder[b.rarity] ?? 99);
      if (r !== 0) return r;
      return a.name.localeCompare(b.name);
    };
    const pool = section === "legends" ? [...LEGEND_CARDS, ...ISRAELI_LEGENDS, ...FRIEND_CARDS]
               : section === "ballon"  ? BALLON_CARDS
               : [...GALAXY_CARDS, ...CARDS];
    let cards;
  ...
  // Stats — per section
  const sectionPool = section === "legends" ? [...LEGEND_CARDS, ...ISRAELI_LEGENDS, ...FRIEND_CARDS]
                    : section === "ballon"  ? BALLON_CARDS
                    : [...GALAXY_CARDS, ...CARDS];
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        {/* 🎯 Section toggle — Players | Legends | Ballon d'Or */}
        <div style={{
          ...
          <button onClick={() => { setSection("ballon"); setFilter("all"); }} style={{
            flex:1,padding:"8px 6px",border:"none",borderRadius:8,
            background: section === "ballon" ? "linear-gradient(135deg,#b8860b,#f5d76e,#b8860b)" : "transparent",
            color: section === "ballon" ? "#1a0a00" : "#94a3b8",
            fontSize:11,fontWeight:900,cursor:"pointer",fontFamily:"inherit",
          }}>🏅 Ballon</button>
        </div>
          ...
          {filteredCards.map(card => {
            ...
            return (
              <div
                ...
                style={{
                  position:"relative",
                  opacity: owned ? 1 : (isBallon ? 1 : 0.3),
                  filter: owned ? "none" : (isBallon ? "none" : "grayscale(1)"),
                  cursor: owned ? "pointer" : "default",
                }}
              >
                {/* Ballon d'Or spoiler — hide card until owned */}
                {isBallon && !owned ? (
                  <div style={{
                    width:"100%",aspectRatio:"2/3",borderRadius:10,
                    background:"linear-gradient(165deg,#1a1200,#2d2000,#1a1200)",
                    border:"2px solid rgba(212,175,55,0.4)",
                    display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
                    gap:6,
                  }}>
                    <div style={{fontSize:28}}>🏅</div>
                    <div style={{fontSize:8,color:"rgba(212,175,55,0.6)",fontWeight:800,letterSpacing:1,textAlign:"center",padding:"0 4px"}}>BALLON D'OR</div>
    ...
    <div style={{padding:"16px 14px 60px",maxWidth:560,margin:"0 auto"}}>
      ...
      <div style={{
        ...
        borderRadius:14,padding:14,
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
            <div style={{fontSize:11,color:"#a855f7",letterSpacing:2,fontWeight:700}}>📊 {t("bonus.topScorersLive")} ({topScorers.length})</div>
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              <div style={{fontSize:9,color:topScorersError?"#fca5a5":"#64748b"}}>
                {topScorersError ? `⚠️ ${topScorersError}` :
                  topScorersFetchedAt ? `${topScorers.length} שחקנים · עודכן לפני ${Math.max(0,Math.round((Date.now()-topScorersFetchedAt)/60000))} דק'` :
                  "לא נטען"}
              </div>
              <button
      ...
      return (
        <div style={{padding:"16px 14px 40px",maxWidth:920,margin:"0 auto"}}>
          ...
          {viewTab === "cards" && (() => {
            ...
            // Full pool including Ballon d'Or
            const BALLON_CARDS = CARDS_BY_RARITY.B;
            const fullPool = [...BALLON_CARDS, ...GALAXY_CARDS, ...CARDS, ...LEGEND_CARDS, ...FRIEND_CARDS, ...ISRAELI_LEGENDS];
            const ownedCards = fullPool.filter(c => (theirCollection[c.id] || 0) > 0);
            // Sort owned by rarity (ballon first, then legends, then best to common, then trash)
            const rarityOrder = { B: -1, X: 0, G: 1, L: 2, E: 3, R: 4, U: 5, C: 6, T: 7 };
            ownedCards.sort((a, b) => {
              const r = (rarityOrder[a.rarity]??99) - (rarityOrder[b.rarity]??99);
              if (r !== 0) return r;
            ...
            const topCard = ownedCards.length > 0
              ...
              : null;
            // Rarity breakdown including B
            const byRarity = ["B","G","L","E","R","U","C"].map(r => ({
...
function AppInner() {
  ...
  const [galaxySpinning, setGalaxySpinning] = useState(false);
  useEffect(() => {
    try {
  ...
  const handleGalaxySpin = () => {
    ...
    setTimeout(() => {
      ...
      setIsSpinning(false);
      setPendingCard(null);
      setGalaxySpinning(false); // 🌌 hide galaxy screen, show reveal
      try {
